...
describe('S3AgentMailbox', () => {
  ...
  test('materializes inbound attachment objects into the workspace inbox', async () => {
    const s3 = new MemoryS3();
    const workspace = fs.mkdtempSync(path.join(os.tmpdir(), 's3-mailbox-workspace-'));
    process.env.NANOCLAW_WORKSPACE_ROOT = workspace;
    s3.putRecord(
      `${sessionPrefix}/inbound/messages/with-attachment.json`,
      'inbound',
      inboundMessage(
        'with-attachment',
        2,
        JSON.stringify({
          text: 'image',
          attachments: [{ name: 'image.png', localPath: 'inbox/with-attachment/image.png' }],
        }),
      ),
    );
    s3.objects.set(`${sessionPrefix}/attachments/with-attachment/image%2Epng`, {
      body: 'png-bytes',
      etag: '"att"',
    });
    const store = new S3AgentMailbox(options, s3);
    try {
      await store.start(key);
      expect(fs.readFileSync(path.join(workspace, 'inbox/with-attachment/image.png'), 'utf8')).toBe('png-bytes');
    } finally {
