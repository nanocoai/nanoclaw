...
- **Setup no longer offers to install the Claude CLI on a Codex or OpenCode install, or before you have chosen a runtime.** The failure-assist dispatcher only knew the runtime picked in the current run, so a failure before the picker (the gateway step) or on a re-run after the checkout was recreated fell back to the Claude path and asked "Claude CLI is needed to diagnose this. Install it now?". Every failure path now reads the selected runtime from the run's pick, the `NANOCLAW_AGENT_PROVIDER` preset, or the `DEFAULT_AGENT_PROVIDER` an earlier run stamped in `.env`; on a non-Claude install it runs that provider's own debugger when registered, otherwise offers Claude only when it is already installed and signed in, and otherwise points at `logs/setup.log` and stops. A fresh run that fails before the runtime picker gets the same guarded outcome. Claude installs keep the install offer from the moment Claude is picked.
