...
Then find the last pnpm global-install `RUN` block (the one that installs `@anthropic-ai/Codex`) and add a new block after it, before `# ---- Entrypoint`:
...
Pinned version matters — `minimumReleaseAge` in `pnpm-workspace.yaml` gates trunk installs, and AGENTS.md requires a fixed ARG version for all Node CLIs installed into the image.
...
**No `TOOL_ALLOWLIST` edit needed.** `container/agent-runner/src/providers/Codex.ts` derives the allow-pattern dynamically from each group's `mcpServers` map (`Object.keys(this.mcpServers).map(mcpAllowPattern)`), so registering `gmail` in Phase 3 automatically allows `mcp__gmail__*`. Earlier versions of this skill instructed a static `TOOL_ALLOWLIST` edit — that's now redundant.
