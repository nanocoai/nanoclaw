...
- **Late button clicks no longer re-wake containers forever.** A click on an `ask_user_question` card that arrives after the tool's poll timeout (default 300s — platform buttons are never disabled and `pending_questions` has no TTL) wrote a `kind='system'` response with the default `trigger=1`. Nothing consumes such a row — the container's main loop filters out system messages — so `countDueMessages` counted it as due on every sweep and the host kept re-waking a container that idled until killed, indefinitely. The response is now written with `trigger: 0` (same pattern as `cli_response`), and `countDueMessages` skips `kind='system'` rows entirely, which also neutralizes already-orphaned rows in existing installs.
