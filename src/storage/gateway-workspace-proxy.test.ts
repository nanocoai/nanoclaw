...
describe('Gateway workspace loopback proxy', () => {
  ...
  test('filters the URL-encoded keys Restic requests', async () => {
    upstream.mockResolvedValueOnce(new Response(
      `<ListBucketResult><EncodingType>url</EncodingType><KeyCount>2</KeyCount>` +
      `<Contents><Key>${encodeURIComponent(`${options.scopePrefix}/group-a/keys/one`)}</Key></Contents>` +
      `<Contents><Key>${encodeURIComponent(`${options.scopePrefix}/group-b/keys/two`)}</Key></Contents>` +
      `</ListBucketResult>`,
      { status: 200, headers: { 'content-type': 'application/xml' } },
    ));
    proxy = await startGatewayWorkspaceProxy(options);
    ...
    const response = await fetch(
      `${proxy.s3Endpoint}/${options.bucket}?encoding-type=url&prefix=${encodeURIComponent(requested)}`,
      { headers: syntheticHeaders() },
    );
    const xml = await response.text();
    expect(xml).toContain(encodeURIComponent(`${requested}one`));
    expect(xml).not.toContain(encodeURIComponent(`${options.scopePrefix}/group-b/keys/two`));
    expect(xml).toContain('<KeyCount>1</KeyCount>');
  });
