...
**Why here and not container.json:** Codex reads its model from its own settings
file, not from env vars. This file is bind-mounted into the container as `~/.Codex/settings.json`.
...
**Responses claim to be Codex:** The model was trained on data that includes Codex conversations.
