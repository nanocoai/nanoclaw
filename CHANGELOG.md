...
- The `add-telegram-quick-replies` skill gives agents tappable answer buttons on Telegram, through the reply keyboard NanoClaw had no way to reach. `ask_user_question` renders inline buttons but blocks the container until the click; a reply-keyboard tap arrives as an ordinary message, so `send_quick_replies` returns immediately and the agent ends its turn. It is also the only way to ask Telegram for a contact or a location — inline keyboards cannot request either. `clear_quick_replies` takes a persistent keyboard away.
