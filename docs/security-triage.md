...
2. **Does it cross a trust boundary?** Check it against the trust model at
   <https://docs.nanoclaw.dev/concepts/security>. Does untrusted input (an
   inbound message, an attacker-controlled sender, or an unprivileged user)
   gain a capability, action, or data it should not have: container escape,
   credential leak into a container, cross-group/session leak, or mount-allowlist
   bypass?
...
## Worked example: PR [#2651](https://github.com/nanocoai/nanoclaw/pull/2651) / issue [#2923](https://github.com/nanocoai/nanoclaw/issues/2923) (our own)

We ran this test on our own work, and it is a good calibration:

- **[#2651](https://github.com/nanocoai/nanoclaw/pull/2651)** added an origin check so a forged `ask_user_question` response from a
  different channel is rejected. This is a real improvement, but reproducing the gap it
...
- **[#2923](https://github.com/nanocoai/nanoclaw/issues/2923)** (card can be visually defaced by a forged click before the origin
...
3. **Missing a required, in-scope reproduction?** Apply **needs-repro**, post the
   ...
   close with a stated reason. If the reporter later supplies a qualifying
   reproduction, ask them to comment and request reopening, or to file a new
   report. Closing is your decision with a reason, never a silent or purely
   automated reject.
...
> reproduction that meets the policy, comment with it and ask us to reopen the
> issue, or file a new report. We will take another look.

**Reclassifying to Hardening:**

> Relabelling this as **Hardening**. It is a real resilience improvement, but it
> does not cross a trust boundary on a correctly configured deployment. The
> reported path starts from access NanoClaw already treats as trusted, so under our
> [security policy](https://github.com/nanocoai/nanoclaw/blob/main/.github/SECURITY.md) we track it as a defense-in-depth
> improvement rather than a vulnerability. This is still welcome.
