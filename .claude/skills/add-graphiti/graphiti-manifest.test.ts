/**
 * Dependency guard for the Graphiti MCP bridge.
 *
 * Agents reach the host-side Graphiti server through `mcp-remote`, a globally
 * installed CLI, so neither an import nor a typecheck can detect its removal.
 * Per-group MCP registration is runtime DB state and the Graphiti stack runs
 * outside the repo; both are verified by the install skill's smoke test.
 */
import fs from 'fs';
...
describe('the Graphiti MCP bridge is installed in the agent image', () => {
  const root = repoRoot();
