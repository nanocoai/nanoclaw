...
- **A `<message>` body that quotes a literal `</message>` is no longer truncated.** `dispatchResultText` parsed the agent's final text with a lazy `<message to="…">…</message>` regex that closed each block at the *first* `</message>` in the content — so whenever an agent quoted the closing tag in its message (e.g. in backticks while explaining message wrapping, or relaying an error about it), the block was silently cut off there and everything after it was dropped to scratchpad, never delivered. `poll-loop.ts` now closes each block at the *last* `</message>` before the next opening tag or end of text, preserving literal tags inside a body.
