...
import { registerApprovalHandler, registerApprovalResolvedHandler } from '../approvals/primitive.js';
...
describe('agent-to-agent human feedback', () => {
  ...
  it('reports the approval wait and a later refusal if the destination is revoked', async () => {
    ...
    expect(notes).toHaveLength(1);
    expect(noteText(notes[0])).toBe(
      "helper's message to researcher is waiting for approval from Alice. It will be delivered if approved.",
    );
  ...
  it('relays an A2A rejection reason directly to the source human', async () => {
    ...
    await createPendingApproval({
      ...
      payload: JSON.stringify({ platform_id: pair.targetId }),
      created_at: now(),
      agent_group_id: pair.sourceId,
