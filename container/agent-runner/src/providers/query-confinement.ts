/**
 * Per-turn query-confinement seam — INERT on pristine core, fail-CLOSED.
 *
 * Some turns are driven by an authenticated EXTERNAL participant (not a board
 * member). Such a turn must run CONFINED: NO built-in fs/bash tools (Read / Bash /
 * Glob / Grep / Write / Edit … are not gated by the nanoclaw MCP gate, so they would
 * be an unguarded exfiltration path over board-private files), NO board
 ...
 * invoked, so pristine-core behaviour is identical to upstream (which has no confined
 * mode).
 */
