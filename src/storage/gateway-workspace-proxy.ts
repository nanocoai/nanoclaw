...
async function handleS3(
  ...
  try {
    ...
    if (isList && response.ok) {
      const body = filterListObjects(await response.text(), requestedPrefix!);
...
function filterListObjects(xml: string, prefix: string): string {
  const encoded = /<EncodingType>url<\/EncodingType>/.test(xml);
  const kept = [...xml.matchAll(/<Contents>[\s\S]*?<\/Contents>/g)]
    .map((match) => match[0])
    .filter((entry) => (encoded ? decodeURIComponent(xmlValue(entry, 'Key')) : xmlValue(entry, 'Key')).startsWith(prefix));
  const without = xml.replace(/<Contents>[\s\S]*?<\/Contents>/g, '');
  const inserted = without.replace('</ListBucketResult>', `${kept.join('')}</ListBucketResult>`);
  return inserted.replace(/<KeyCount>\d+<\/KeyCount>/, `<KeyCount>${kept.length}</KeyCount>`);
}
