...
<style>
  :root {
    ...
    --accent: #087a80; --accent-2: #45c6c0; --accent-soft: rgba(15, 154, 160, 0.10); --on-accent: #ffffff;
    --you: #96500d; --you-soft: rgba(217, 130, 43, 0.12); --danger: #bc302c; --danger-soft: rgba(214, 69, 65, 0.10); --on-danger: #ffffff; --ok: #168747;
    --shadow: 0 20px 64px rgba(20, 24, 40, 0.08), 0 2px 8px rgba(20, 24, 40, 0.03);
    --gutter: clamp(12px, 3vw, 24px);
    --safe-top: max(var(--gutter), env(safe-area-inset-top));
    --safe-bottom: max(var(--gutter), env(safe-area-inset-bottom));
  }
  @media (prefers-color-scheme: dark) {
    :root {
      --bg: #0d0f14; --card: #161920; --ink: #eceef4; --muted: #9aa2b6; --line: #262b36;
      --accent: #34c9c6; --accent-2: #7fe0d3; --accent-soft: rgba(52, 201, 198, 0.12); --on-accent: #082c2f;
      --you: #f0a95a; --you-soft: rgba(240, 169, 90, 0.12); --danger: #ff8b83; --danger-soft: rgba(255, 111, 102, 0.12); --on-danger: #3b100e; --ok: #4cd67f;
      --shadow: 0 20px 64px rgba(0, 0, 0, 0.3);
    }
  }
  * { box-sizing: border-box; }
  ...
  body {
    ...
    padding: var(--safe-top) max(var(--gutter), env(safe-area-inset-right)) var(--safe-bottom) max(var(--gutter), env(safe-area-inset-left));
  }
  .card {
    width: 100%; max-width: 520px; min-width: 0; margin: auto; background: var(--card); border: 1px solid var(--line); border-radius: 28px; box-shadow: var(--shadow);
    max-height: calc(100vh - var(--safe-top) - var(--safe-bottom));
    max-height: calc(100dvh - var(--safe-top) - var(--safe-bottom));
    padding: 28px clamp(18px, 5vw, 32px) 20px; display: flex; flex-direction: column; gap: 16px;
  }
  .card > * { flex-shrink: 0; min-width: 0; }
  ...
  h1 { font-size: clamp(23px, 5vw, 26px); line-height: 1.2; font-weight: 700; margin: 4px 0 0; letter-spacing: -0.035em; }
  .tagline { font-size: 14px; line-height: 1.5; color: var(--muted); margin: 0; max-width: 34ch; }
  .agentline { font-size: 13px; font-weight: 600; color: var(--accent); min-height: 20px; max-width: 100%; overflow-wrap: anywhere; }
  .stage { display: grid; place-items: center; gap: 12px; padding: 6px 0 2px; }
  canvas { width: 240px; max-width: 100%; height: 96px; display: block; }
  .state {
    display: inline-flex; align-items: center; gap: 8px; padding: 6px 12px; border-radius: 999px; font-size: 13px; font-weight: 600;
    background: var(--accent-soft); color: var(--accent); min-height: 32px; max-width: 100%; text-align: center; overflow-wrap: anywhere; transition: background .2s, color .2s;
  }
  ...
  .status { text-align: center; font-size: 14px; color: var(--muted); min-height: 21px; max-height: 84px; overflow-y: auto; overflow-wrap: anywhere; margin: 0; }
  .status.err { color: var(--danger); }
  .conversation { display: flex; flex-direction: column; flex: 0 1 auto; min-height: 0; border-top: 1px solid var(--line); }
  .conversation-head { display: flex; align-items: center; justify-content: space-between; gap: 8px; min-height: 44px; flex: none; }
  .conversation-head h2 { margin: 0; font-size: 12px; font-weight: 600; color: var(--muted); }
  #latest { padding: 0 8px; min-height: 44px; font-size: 12px; background: transparent; color: var(--accent); }
  .transcript { display: flex; flex-direction: column; gap: 10px; min-height: 0; max-height: 240px; overflow-y: auto; overscroll-behavior-y: contain; scrollbar-width: thin; scrollbar-color: var(--line) transparent; padding: 2px 4px 4px; }
  .b { flex: none; max-width: 88%; padding: 10px 14px; border-radius: 16px; font-size: 15px; line-height: 1.5; overflow-wrap: anywhere; white-space: pre-wrap; }
  ...
  .bar {
    display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 10px 12px; border-top: 1px solid var(--line); padding-top: 16px;
  }
  .timer { grid-column: 1 / -1; font-variant-numeric: tabular-nums; font-size: 12px; line-height: 1; color: var(--muted); text-align: center; }
  .timer:empty { display: none; }
  button {
    font: inherit; font-weight: 650; border: 0; border-radius: 999px; cursor: pointer; min-height: 48px; min-width: 44px; touch-action: manipulation;
    transition: transform .12s cubic-bezier(.23, 1, .32, 1), opacity .15s ease, background .15s ease;
  }
  button:active:not(:disabled) { transform: scale(.97); }
  :focus-visible { outline: 3px solid var(--accent); outline-offset: 3px; }
  .transcript:focus-visible { outline-offset: -3px; border-radius: 8px; }
  button:disabled { opacity: .45; cursor: default; }
  #call { background: var(--accent); color: var(--on-accent); padding: 12px 20px; font-size: 16px; }
  #call.hang { background: var(--danger); color: var(--on-danger); }
  #mute { background: var(--line); color: var(--ink); padding: 10px 16px; min-width: 112px; display: inline-flex; justify-content: center; align-items: center; gap: 8px; font-size: 14px; }
  #mute.on { background: var(--danger-soft); color: var(--danger); }
  .meter { display: inline-flex; flex: none; gap: 2px; align-items: flex-end; height: 14px; color: var(--you); }
  ...
  footer { text-align: center; font-size: 12px; color: var(--muted); overflow-wrap: anywhere; }
  @media (hover: hover) and (pointer: fine) { button:hover:not(:disabled) { filter: brightness(.94); } }
  @media (max-height: 700px) {
    .card { padding-top: 18px; padding-bottom: 14px; gap: 12px; }
    .brand { display: grid; grid-template-columns: 56px minmax(0, 1fr); gap: 6px 14px; text-align: left; }
    .brand > :not(.logo) { grid-column: 2; }
    .logo { grid-row: 1 / 4; width: 56px; height: 56px; border-radius: 16px; }
    h1 { margin: 0; }
    .tagline { font-size: 13px; }
    canvas { height: 64px; }
    .stage { gap: 6px; padding: 0; }
  }
  @media (max-height: 560px) {
    .tagline { display: none; }
    .logo { grid-row: 1 / 3; }
    .stage { display: flex; justify-content: center; gap: 12px; }
    canvas { width: 96px; height: 40px; flex: none; }
    .bar { grid-template-columns: auto minmax(0, 1fr) auto; gap: 8px; padding-top: 10px; }
    .timer { grid-column: auto; min-width: 40px; }
    #call { grid-column: 2; }
    #mute { min-width: 100px; padding-inline: 12px; }
  }
  @media (max-height: 500px) and (min-width: 480px) {
    .card { max-width: 800px; display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1.4fr); grid-template-rows: auto auto minmax(0, 1fr) auto; grid-template-areas: 'brand stage' 'brand status' 'brand conversation' 'foot bar'; gap: 8px 24px; padding: 16px 20px; }
    .brand { grid-area: brand; display: flex; justify-content: center; text-align: center; gap: 10px; }
    .stage { grid-area: stage; }
    .status { grid-area: status; max-height: 42px; }
    .conversation { grid-area: conversation; }
    .transcript { max-height: 120px; }
    .bar { grid-area: bar; }
    footer { grid-area: foot; align-self: center; }
  }
  @media (prefers-reduced-motion: reduce) {
    .dots i { animation: none; opacity: 1; }
    button, .logo .dot, .state, .meter i { transition: none; }
...
<main class="card">
  ...
  <p class="status" id="status" role="status">Allow the microphone when asked.</p>
  <section class="conversation" id="conversation" aria-label="Conversation" hidden>
    <div class="conversation-head"><h2>Live transcript</h2><button id="latest" type="button" hidden>Jump to latest &darr;</button></div>
    <div class="transcript" id="transcript" role="log" aria-label="Live transcript" aria-live="polite" tabindex="0"></div>
  </section>
  <div class="bar">
    <span class="timer" id="timer" role="timer" aria-label="Call duration"></span>
...
(function () {
  ...
  var callBtn = $('call'), muteBtn = $('mute'), muteText = $('mute-text'), statusEl = $('status'), stateEl = $('state'),
      ...
      conversation = $('conversation'), latestBtn = $('latest');
  var motionQuery = window.matchMedia('(prefers-reduced-motion: reduce)'), reduced = motionQuery.matches;
  ...
  function setStatus(text, err) { statusEl.textContent = text; statusEl.className = 'status' + (err ? ' err' : ''); statusEl.tabIndex = err ? 0 : -1; }
  function setState(s, text) {
    ...
    if (s === 'error') dot.className = 'dot err';
    if (phase === 'live') {
      var hints = { listening: 'Go ahead. I\u2019m listening.', thinking: 'Your agent is working on it.', speaking: 'You can interrupt at any time.' };
      if (hints[s]) setStatus(muted ? 'Your microphone is muted.' : hints[s]);
    }
  }
  ...
  // Follow new speech only while the caller is already reading the latest turn.
  function atLatest() { return transcript.scrollHeight - transcript.scrollTop - transcript.clientHeight < 32; }
  transcript.addEventListener('scroll', function () { latestBtn.hidden = atLatest(); });
  latestBtn.addEventListener('click', function () { transcript.scrollTop = transcript.scrollHeight; latestBtn.hidden = true; transcript.focus({ preventScroll: true }); });
  function caption(who, delta) {
    if (!delta) return;
    var follow = conversation.hidden || atLatest();
  ...
  async function start() {
    phase = 'connecting'; callBtn.disabled = true; transcript.innerHTML = ''; conversation.hidden = true; latestBtn.hidden = true; lastWho = ''; lastBubble = null; timerEl.textContent = '';
    ...
    try {
      ...
      pc.onconnectionstatechange = function () {
        if (pc.connectionState === 'connected' && phase === 'connecting') {
          ...
          muteBtn.disabled = false; dot.className = 'dot live'; setState('listening');
  ...
  muteBtn.addEventListener('click', function () {
    ...
    setState(agentState, stateText.textContent);
  });
