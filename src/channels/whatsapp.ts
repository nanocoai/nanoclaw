...
const waEnv = readEnvFile(['ASSISTANT_NAME', 'ASSISTANT_HAS_OWN_NUMBER', 'WHATSAPP_DM_ALLOWLIST']);
...
// Optional DM allowlist — comma-separated bare phone numbers (no @s.whatsapp.net
// suffix). When set, only these numbers (plus the linked account's own number,
// via self-chat) can reach the bot in a 1:1 chat; everyone else is silently
// ignored before any router/approval flow sees them. Unset = no restriction
// (existing behavior). Group chats are never filtered by this list.
const WHATSAPP_DM_ALLOWLIST: Set<string> = new Set(
  (waEnv.WHATSAPP_DM_ALLOWLIST || '')
    .split(',')
    .map((n) => n.trim())
    .filter(Boolean),
);
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // DM allowlist gate — before onMetadata/onInbound so unlisted
            // numbers never create a users row, never trigger the
            // channel-registration approval card, and leave no trace.
            if (
              !isGroup &&
              WHATSAPP_DM_ALLOWLIST.size > 0 &&
              chatJid !== botPhoneJid &&
              !WHATSAPP_DM_ALLOWLIST.has(chatJid.split('@')[0])
            ) {
              continue;
            }
