...
It is the reporting-and-triage policy. For the architecture it builds on, see
the security overview at
**<https://docs.nanoclaw.dev/concepts/security>** (the canonical threat model),
with operational details in the [hardening guide](https://docs.nanoclaw.dev/operate/hardening)
and [credentials guide](https://docs.nanoclaw.dev/operate/credentials). That page
defines what NanoClaw trusts and distrusts; this one defines how we handle
reports against those boundaries.
...
updated as we triage, fix, and coordinate disclosure. Valid reports are
...
- **Your setup**, if it matters: channel type, provider, container runtime, and
  the affected agent group or session.
...
- A container escape, such as agent code reaching the host or another group's files or
...
- Data crossing an isolation boundary: one agent group reading or acting on
  another group's data, or one session's data leaking into another.
...
- A **hardening** (defense-in-depth) improvement makes NanoClaw more resilient
  _after another boundary has already failed_, or reduces risk without a
...
everyone, and we may close them with a short pointer to this policy. Repeatedly
...
**Hardening**) or close it with a comment explaining why. If you later have a
reproduction that meets this policy, comment with it and ask us to reopen the
issue, or file a new report. Closing is a maintainer decision with a stated
reason, not an automated rejection. Genuinely sensitive findings belong in a
