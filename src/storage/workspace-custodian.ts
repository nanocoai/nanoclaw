...
export class WorkspaceCustodian {
    ...
    try {
      ...
      if (replica.state === "publishing") {
        ...
        if (result.code !== 0 && replica.base === null) {
          ...
          // command. The bootstrap is the failure; say so — UNLESS init says
          // the repository already exists: then the bootstrap was never the
          // problem and the BACKUP error is the one a reader needs.
          else
            throw new Error(
              `${commandError("backup", result).message}; the repository exists (${commandError("init", init).message})`,
            );
        }
  ...
  async ensureRepository(groupId: string): Promise<'created' | 'present'> {
    ...
    if (init.code !== 0) {
      // The init is only the EXISTENCE arm of this probe. When it says the
      // repository already exists, the failure that matters is the PROBE's —
      // the repository is there and this custodian cannot open it — and
      // throwing init's line buried exactly that on nancy-v3 (2026-08-30):
      // three generations crash-looped on "already initialized" while the
      // probe's own error was never printed once.
      throw new Error(
        `${commandError("cat config", probe).message}; the repository exists (${commandError("init", init).message})`,
      );
    }
    return 'created';
  }
