...
import { buildContainerArgs, buildMounts, resolveProviderContribution } from './container-runner.js';
...
export async function preflightContainerConfig(
  ...
  try {
    ...
    const ephemeralSession = { id: token, agent_provider: null };
    ...
    const { provider, contribution } = resolveProviderContribution(ephemeralSession, ephemeralGroup, candidate);
    ...
    let stdout = '';
    let stderr = '';
    const appendOutput = (output: string, data: Buffer): string =>
      output + data.toString('utf8', 0, 64 * 1024 - output.length);
    child.stdout?.on('data', (data) => (stdout = appendOutput(stdout, data)));
    child.stderr?.on('data', (data) => (stderr = appendOutput(stderr, data)));
    ...
    const exitCode = await new Promise<number>((resolve, reject) => {
      ...
      child.once('close', (code) => {
        clearTimeout(timer);
        if (timedOut) reject(new Error(`preflight container timed out after 120s\n${stderr}`));
        else resolve(code ?? 1);
      });
    });
    const providerOutput = `${stdout}${stderr}`.trim();
    if (exitCode !== 0)
      throw new Error(`preflight container exited with code ${exitCode}: ${providerOutput || 'no output'}`);
    return { providerOutput, exitCode };
  } finally {
    for (const tempPath of [root, path.dirname(sessDir)]) {
      try {
