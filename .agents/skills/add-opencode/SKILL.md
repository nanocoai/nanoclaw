...
NanoClaw runs agents in a long-lived **poll loop** inside the container. The backend is selected with **`AGENT_PROVIDER`** (`Codex` | `opencode` | `mock`).

Trunk ships with only the `Codex` provider baked in. This skill copies the OpenCode provider files in from the `providers` branch, wires them into the host and container barrels, installs dependencies, and rebuilds the image.
...
```dockerfile
    pnpm install -g \
        "@anthropic-ai/Codex@${CLAUDE_CODE_VERSION}" \
...
OPENCODE_MODEL=openrouter/anthropic/Codex-sonnet-4
OPENCODE_SMALL_MODEL=openrouter/anthropic/Codex-haiku-4.5
...
OPENCODE_MODEL=anthropic/Codex-sonnet-4-20250514
OPENCODE_SMALL_MODEL=anthropic/Codex-haiku-4-5-20251001
```
...
Extra MCP servers still come from **`NANOCLAW_MCP_SERVERS`** / `container_config.mcpServers` on the host; the runner merges them into the same `mcpServers` object passed to **both** Codex and OpenCode providers.
