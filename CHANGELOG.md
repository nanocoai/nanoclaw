...
- **Native iMessage via Photon — new `imessage-cloud` channel (`/add-imessage-cloud`).** A skill-installable native channel that connects NanoClaw to iMessage through [Photon](https://photon.codes)'s managed Spectrum platform over the `spectrum-ts` gRPC stream, in-process on the host — no Mac relay, no webhook. `/add-imessage-cloud` fetches the adapter from the `channels` branch and runs `scripts/photon-setup.ts`, a device-login wizard that auto-provisions the Photon project, secret, phone, and iMessage line into `.env`. Coexists with the legacy Chat-SDK `imessage` channel (distinct registry key) and is the iMessage option offered in first-run `/setup`. Both directions, markdown, attachments, tapback reactions, read receipts, typing, and `ask_user_question` via slash-command replies. See [docs/imessage-cloud.md](docs/imessage-cloud.md).
