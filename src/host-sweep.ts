...
async function sweepSession(session: Session): Promise<void> {
  ...
  try {
    ...
    // BEFORE the due-count below (e.g. an install-overlay's idle/stale-board runner suppression)
    ...
    // AND atomic (rolled back on throw). No gates registered on pristine core ⇒ this is a no-op.
    ...
    // falls back to the global TIMEZONE inside handleRecurrence. No resolver registered on
    // pristine core ⇒ undefined ⇒ global TIMEZONE for all rows.
