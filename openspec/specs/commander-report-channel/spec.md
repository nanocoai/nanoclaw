...
定义 Commander 派工任务的受限汇报通道、汇报状态枚举、message loop 投递方式，以及目标群一轮结束后的自动终态兜底。
...
`report_to_source` 与兼容旧名 `report_to_main` 的 status SHALL 限定为 `progress` / `done` / `blocked` / `failed` / `question` 五种之一。payload SHALL 固定字段 `status` / `summary` / `details?` / `artifacts?`，SHALL NOT 含 task_id 参数（task_id 由 host 用 reporting_group 锁定）。
...
- **THEN** host SHALL path.resolve 校验通过后记入账本 artifacts 字段，发起群可直接按宿主机路径读取，不经飞书云盘
