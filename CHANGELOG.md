...
- **A resumed conversation is now told when its memory went stale.** Memory is injected only when a provider opens a new context window (`startup`, `clear`, `compact`), never on `resume` — and because a continuation survives container restarts (including the 30-minute idle sweep and `ncl groups restart`), an agent could keep answering from a copy of `memory/` that another session had since rewritten, with no restart able to fix it. The memory hook now stamps a per-session marker, and a resumed turn opens with a `<memory_changed>` line naming whichever of the loaded memory files is newer, so the agent re-reads instead of reciting. Nothing is emitted when nothing changed, and the agent's own edits are never reported back to it.
