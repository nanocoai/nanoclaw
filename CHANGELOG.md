...
- **The skill refresh now tells the truth about local adapter state** (#3529). Three fixes: a barrel import shipped by a sibling skill's copy directive (e.g. `slack-a2a-guard`, copied and appended by `/add-slack`) is attributed to that sibling instead of failing the refresh as a phantom `add-<name>` skill; a hand-written adapter with no skill anywhere is reported as `unmanaged` and left untouched instead of blocking the whole refresh (explicitly requesting it by name still errors); and each refreshed skill reports `localDiffDiscarded` — payload files the refresh overwrote whose local content no registry commit ever shipped, so deliberately patched adapters are called out instead of being silently reverted. Stock upgrades (an old registry version advancing) are not flagged.
