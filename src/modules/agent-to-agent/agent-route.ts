...
import { approverLabel, buildAddAgentDestinationCommand, notifySource } from './notify-source.js';
...
export async function routeAgentMessage(
  ...
  if (decision.effect === 'deny') {
    ...
    if (!target) {
      ...
      text = `${sourceName} tried to send a message to "${target.name}", but it was blocked: ${sourceName} does not have "${target.name}" in its destinations. To allow this, an admin can run: ${command}. After that, ask ${sourceName} to resend.`;
    }
    await notifySource(session.id, text);
    return;
  }
  ...
  if (decision.effect === 'hold') {
    ...
    await notifySource(
      session.id,
      `${sourceName}'s message to ${targetName} is waiting for approval from ${await approverLabel(result.approverUserId)}. It will be delivered if approved.`,
    );
    return;
  }
...
async function performAgentRoute(
  ...
  if (targetAgentGroupId !== session.agent_group_id && !targetCanReply && !oneWayNoticeKeys.has(noticeKey)) {
    ...
    const notified = await notifySource(
      session.id,
      `Delivered to ${targetName}. Note: ${targetName} cannot reply — it does not have ${sourceName} in its destinations. To enable replies, an admin can run: ${command}.`,
    );
    if (notified) oneWayNoticeKeys.add(noticeKey);
  }
}
