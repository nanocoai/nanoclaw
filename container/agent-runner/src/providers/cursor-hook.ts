 ...
 * `memoryContextForSessionStart`. sessionStart takes `additional_context`.
 * preCompact only forwards `user_message`, so compact returns that field.
 *
...
export function hookPayload(
  ...
  baseDir?: string,
): { additional_context?: string; user_message?: string } {
  const context = memoryContextForSessionStart(source, baseDir);
  if (!context) return {};
  // Cursor's preCompact response copies only user_message into the compaction
  // request. additional_context is the sessionStart field and is dropped here.
  if (source === 'compact') return { user_message: context };
