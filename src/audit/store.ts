...
class CentralPostgresAuditStore implements AuditStore {
  ...
  private async lockDeliveryState(): Promise<DeliveryStateRow> {
    // The producer row FIRST, because delivery_state.host_id references it.
    // `append` already does this in the right order, but retention maintenance
    // reaches lockDeliveryState on a Host that has not appended yet — on a
    // fresh install nothing has, so the parent table is empty and every
    ...
    //
    // once a minute, for a Host whose audit log was simply still empty.
    await this.db.run(
      `INSERT INTO host_audit_producer_state (host_id, last_seq)
       VALUES (?, 0)
       ON CONFLICT (host_id) DO NOTHING`,
