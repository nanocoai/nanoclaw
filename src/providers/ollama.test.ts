...
import '../provider-contracts/ollama.js';
import { getProviderHostContract } from '../provider-contracts/registry.js';
import { BLOCKED_CLOUD_HOSTS, readOllamaModelState, resolveOllamaWebBrowsing } from './ollama.js';
...
describe('ollama host provider configuration', () => {
  it('routes only through the local daemon and blocks direct cloud hosts', async () => {
    ...
    // Blocked hosts live on the host contract: core keeps only env from the
    // adapter once a contract is registered.
    expect(config.blockedHosts).toBeUndefined();
    expect(getProviderHostContract('ollama')?.blockedHosts).toEqual(BLOCKED_CLOUD_HOSTS);
    expect(BLOCKED_CLOUD_HOSTS).toEqual([
      'api.anthropic.com',
      'claude.ai',
      'www.claude.ai',
