...
exit code 2 from the CLI means the `api.typesafe.ai` credential is not
connected, not a bug. Verify from a chat with any agent:

> Using typesafe-judge, ask one noul question: is "the container never starts" a bug report? Show the raw answer.

To stamp an agent that uses this tool for triage, PR routing, dedupe and backlog
ranking, see the `maintainer` template in [docs/typesafe-judge.md](../../../docs/typesafe-judge.md).
...
**An agent gets exit code 2 (401/403).** The gateway has no credential for
