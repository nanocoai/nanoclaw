...
- [BREAKING] **`ncl roles grant`/`revoke` require an explicit `--scope`.** `--role admin` without a scope no longer silently creates a *global* admin (admin over every agent group). Both commands now take a required `--scope global|group`: `--scope global` forbids `--group`; `--scope group` requires it; `owner` is always global. Grant/revoke also return a plain-language summary of the resulting scope and capabilities. **Migration:** any script or agent instruction that ran `ncl roles grant --user <u> --role admin` relying on the silent-global default must pass `--scope global` (all groups) or `--scope group --group <agent_group_id>` (one group). See [docs/role-scope-migration.md](docs/role-scope-migration.md).
