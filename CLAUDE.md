...
- **Adding a non-npm CLI the agent invokes at runtime** (a prebuilt release binary like `rtk`) → append a `{ "name", "version", "binary": { "amd64": { "url", "sha256" }, "arm64": { … } } }` entry to the same `container/cli-tools.json`. `install-cli-tools.sh` fetches the archive for the image's arch, verifies the SHA-256, and installs it `0755` into `/usr/local/bin` — the only place it is on the container PATH. The checksum is what an exact npm version is for the entries above, so never point a binary entry at a URL that can move (a `latest` redirect, a branch tarball, an installer script). Don't mount a host binary in instead: additional mounts are rewritten to `/workspace/extra/<basename>`, which is not on PATH.
