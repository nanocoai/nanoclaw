...
- **Channel/provider install skills** — copy the relevant module(s) in from the `channels` or `providers` branch, wire imports, install pinned deps (e.g. `/add-discord`, `/add-slack`, `/add-whatsapp`, `/add-opencode`). User-invoked only (`disable-model-invocation: true`; `agents/openai.yaml` with `policy.allow_implicit_invocation: false` for Codex), so they are not loaded implicitly — do not improvise their steps. Install one with `bin/ncl skills apply <name>` (`plan` previews it; `--restart` reloads the service; host down: `pnpm exec tsx scripts/skill-headless.ts apply <name>`). Respect deployment refusals: do not retry through a direct script or manual source edits. For `needs-setup`, report the pending operator steps and stop before wiring or restarting. Only in an operator-enabled source checkout, a prose-only skill can be handed to the operator as `/<name>`.
