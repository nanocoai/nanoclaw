...
This skill adds a stdio-based MCP server that exposes models running in the local [Atomic Chat](https://github.com/AtomicBot-ai/Atomic-Chat) desktop app as tools for the container agent. Codex remains the orchestrator but can offload work to local models served by Atomic Chat on `http://127.0.0.1:1337/v1` (OpenAI-compatible).
...
The skill ships the MCP server source in this folder and copies it into the agent-runner tree at install time, then wires it up with small edits to `index.ts`, `providers/Codex.ts`, and `container-runner.ts`. No branch merge — all edits are additive and idempotent.
...
```bash
cp .Codex/skills/add-atomic-chat-tool/atomic-chat-mcp-stdio.ts container/agent-runner/src/atomic-chat-mcp-stdio.ts
```
...
Edit `container/agent-runner/src/providers/Codex.ts`. Find `'mcp__nanoclaw__*',` in the `TOOL_ALLOWLIST` array and add `'mcp__atomic_chat__*',` on the following line:
...
```bash
# Atomic Chat MCP tool (.Codex/skills/add-atomic-chat-tool)
...
3. The allowlist wasn't updated — check `container/agent-runner/src/providers/Codex.ts` includes `mcp__atomic_chat__*` in `TOOL_ALLOWLIST`
