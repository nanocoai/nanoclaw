...
Installs [mnemon](https://github.com/mnemon-dev/mnemon) in the agent container image. On each container start, `mnemon setup` registers Codex hooks that surface relevant memory before the agent responds and store new insights after each turn. Memory is written to the per-agent-group `.Codex/` mount and survives container restarts.

## Provider Compatibility

**mnemon hooks only work with `--target Codex`.** If the agent group uses `AGENT_PROVIDER=opencode`, hooks registered by `mnemon setup` will never fire — OpenCode spawns its own process and doesn't invoke the `Codex` CLI at all.
...
- `AGENT_PROVIDER=Codex` (default) — fully compatible, proceed with both Phase 2 steps.
...
## Phase 2: Apply Changes (Codex path)
...
ENV MNEMON_DATA_DIR=/home/node/.Codex/mnemon
```

`MNEMON_DATA_DIR` points into the per-agent-group `.Codex/` mount so memory persists across container restarts. No extra volume mounts needed.
...
mnemon setup --target Codex --yes --global >/dev/stderr 2>&1
...
docker exec $(docker ps --filter name=nanoclaw-v2 --format '{{.Names}}' | head -1) \
  cat /home/node/.Codex/settings.json | grep -A5 mnemon
```
...
**What this means for the agent:** The agent (running inside OpenCode) can call `mnemon recall`, `mnemon remember`, `mnemon link`, and `mnemon status` via its bash tool. mnemon writes its graph to `$MNEMON_DATA_DIR`, which is in the per-agent-group `.Codex/` mount — so memory persists across container restarts.
...
Mnemon writes to `/home/node/.Codex/mnemon/` inside the container, which maps to the per-agent-group `.Codex/` directory on the host. To find the exact host path:

```bash
docker inspect $(docker ps --filter name=nanoclaw-v2 --format '{{.Names}}' | head -1) \
  --format '{{range .Mounts}}{{if eq .Destination "/home/node/.Codex"}}{{.Source}}{{end}}{{end}}'
```
...
`mnemon setup` writes hooks into `/home/node/.Codex/settings.json`. Verify:

```bash
docker exec <container> cat /home/node/.Codex/settings.json
```
...
```bash
docker exec -it <container> mnemon setup --target Codex --yes --global
```
