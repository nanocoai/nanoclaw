...
- [BREAKING] **Chat SDK and channel adapters are updated to `4.35.0`.** Telegram now preserves Bot API 10.1 rich-message text and media instead of delivering an empty inbound message. The bridge and every installed adapter must keep the same exact version. **Migration:** re-run each installed `/add-<channel>` skill after updating, then rebuild and restart NanoClaw.
