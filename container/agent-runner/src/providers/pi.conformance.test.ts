/**
 * Runtime-contract conformance for the pi payload, run through the core's
 ...
 * there — the same core detection codex.conformance.test.ts uses. The
...
if (hasContractCore) {
  ...
  const { piRuntimeContract } = await import('../provider-contracts/pi.js');
  const { defineProviderConformance } = await import('../provider-contracts/testing/conformance.js');
  defineProviderConformance('pi', piRuntimeContract);
} else {
  describe.skip('runtime provider contract: pi', () => {
