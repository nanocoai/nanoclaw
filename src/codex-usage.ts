...
export interface CodexUsageResult {
  ...
  error?: 'not_codex' | 'no_session' | 'no_data';
}

/** 在 CODEX_HOME/sessions 下递归找最新(mtime)的 rollout-*.jsonl */
...
export function getCodexUsage(group: RegisteredGroup): CodexUsageResult {
  if (resolveCliMode(group.containerConfig) !== 'codex') {
    return { rateLimits: null, error: 'not_codex' };
  }
  ...
  logger.info(
    { group: group.folder, rollout: path.basename(rollout), plan: extracted.planType },
    'codex usage 读取成功',
  );
  return { rateLimits, planType: extracted.planType };
}

/** 格式化 codex 配额输出(复用 Claude usage 的进度条/重置时间样式) */
export function formatCodexUsage(result: CodexUsageResult): string {
  if (result.error === 'not_codex') {
    return '⚠️ 当前群不是 codex 模式，/usage 走 Claude 配额查询';
  }
