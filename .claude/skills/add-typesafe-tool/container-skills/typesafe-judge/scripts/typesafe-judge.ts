...
/**
 * Longer than the 120 s a NanoClaw gateway approval card stays open, so on Iron
 * Proxy one call outlives one card instead of timing out and raising a second.
 */
export const DEFAULT_TIMEOUT_MS = 150_000;
/** Ceiling on questions per call. Fan-out is cheap, not free: this keeps a loop from quietly inflating spend. */
...
export interface ScoreAnswer {
  type: 'score';
  score: number;
  /** Index → the level as it was asked: a string, or TypeSafe's structured level description. */
...
/** The gateway's spend ceiling or TypeSafe's rate limit, not clearing within this call's patience. Loops must stop on it. */
...
Flags:
  ...
  --timeout <seconds>       Per-request timeout (default ${DEFAULT_TIMEOUT_MS / 1000}; outlives one 120 s approval card)
  --max-questions <n>       Refuse a call with more questions than this (default ${DEFAULT_MAX_QUESTIONS})
  -h, --help                This text

Exit codes: 0 ok, 1 usage/input, 2 refused (401 credential missing; 403 policy, permission or quota),
            3 upstream failure, 5 rate limited (gateway spend ceiling or plan limit): stop any loop.
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case '--timeout':
        ...
        break;
      case '--max-questions':
        opts.maxQuestions = Math.floor(parseNumber(a, argv[++i], 1, 500));
        break;
      default:
...
export function buildRequest(opts: CliOptions, body: string | undefined): JudgeRequest {
  ...
  if (count > opts.maxQuestions) {
    throw new UsageError(
      `${count} questions exceed the per-call ceiling of ${opts.maxQuestions}. Drop the ones you will not use, ` +
        'or raise it deliberately with --max-questions.',
    );
  }
...
export function gateAnswer(answer: Answer, t: GateThresholds = DEFAULT_THRESHOLDS): Gate {
  switch (answer.type) {
    ...
    case 'score': {
      ...
      const entry = answer.legend[String(index)];
      return {
        decision: decide(answer.confidence, t.act, t.propose),
        certainty: answer.confidence,
        value: answer.score,
        level: { index, text: typeof entry === 'string' ? entry : entry === undefined ? '' : JSON.stringify(entry) },
      };
    }
  }
}
...
/** A probability map over exactly the expected keys, each in [0, 1], summing to 1 within rounding. */
function isDistribution(v: unknown, keys: string[]): v is Record<string, number> {
  if (!isRecord(v)) return false;
  const got = Object.keys(v);
  if (got.length !== keys.length || !keys.every((k) => k in v)) return false;
  const values = Object.values(v);
  if (!values.every(unit)) return false;
...
export function answerMatches(question: Question, v: unknown): v is Answer {
  ...
  if (v.type === 'choice') {
    ...
    if (typeof v.choice !== 'string' || !options.includes(v.choice) || !unit(v.confidence)) return false;
    if (!isDistribution(v.probabilities, options)) return false;
    // The reported choice must be the most probable option (ties allowed): a
    // choice that disagrees with its own distribution is a malformed answer.
    const top = Math.max(...Object.values(v.probabilities));
    return v.probabilities[v.choice] >= top - 1e-9;
  }
  if (v.type === 'score') {
    const levels = Array.isArray(question.criteria) ? question.criteria.length : 0;
    const keys = Array.from({ length: levels }, (_, i) => String(i));
    if (typeof v.score !== 'number' || !Number.isFinite(v.score) || v.score < 0 || v.score > Math.max(0, levels - 1)) return false;
    if (!unit(v.confidence) || !isDistribution(v.probabilities, keys)) return false;
    // The legend must restate exactly the offered levels, in order: gate.level
    // reads from it, and a legend that disagrees with the criteria (reversed,
    // renamed) would label a level with the wrong text and still gate as `act`.
    // Levels may be strings or TypeSafe's structured level descriptions.
    const criteria = question.criteria as unknown[];
    if (!isRecord(v.legend) || Object.keys(v.legend).length !== levels) return false;
    if (!keys.every((k) => JSON.stringify((v.legend as Record<string, unknown>)[k]) === JSON.stringify(criteria[Number(k)]))) return false;
    // A score is the probability-weighted level. One that disagrees with its own distribution is malformed.
    const probabilities = v.probabilities;
    const weighted = keys.reduce((sum, k) => sum + Number(k) * probabilities[k], 0);
    return Math.abs(weighted - v.score) <= 0.05;
  }
  return false;
}
...
export async function evaluate(
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    if (RETRY_STATUSES.has(res.status)) {
      const wait = retryAfterMs(res.headers.get('retry-after'), Date.now(), Number.POSITIVE_INFINITY);
...
export function retryAfterMs(header: string | null, now = Date.now(), cap = MAX_RETRY_AFTER_MS): number {
  if (!header) return 0;
  const seconds = Number(header);
  const ms = Number.isFinite(seconds) ? seconds * 1000 : Date.parse(header) - now;
  return Number.isFinite(ms) && ms > 0 ? Math.min(ms, cap) : 0;
}
...
export async function run(argv: string[], io: RunIo): Promise<number> {
  ...
  try {
    ...
    if (err instanceof RateLimitError) {
      io.stderr(`error: ${message}\nStop the loop you are in and report how far you got. Do not retry until the window has passed.\n`);
      return 5;
    }
    io.stderr(`error: ${message}\n`);
    return 3;
