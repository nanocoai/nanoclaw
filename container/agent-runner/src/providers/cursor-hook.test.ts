...
describe('cursor-hook adapter', () => {
  ...
  it('renders the shared NanoClaw memory context on startup and clear', () => {
    ...
    try {
      ...
      for (const source of ['startup', 'clear'] as const) {
        const payload = hookPayload(source, base);
        expect(payload.additional_context).toContain('Cursor memory sentinel');
  ...
  it('returns user_message on compact, the field Cursor forwards into preCompact', () => {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), 'cursor-memory-hook-'));
    try {
      fs.mkdirSync(path.join(base, 'memory', 'system'), { recursive: true });
      fs.writeFileSync(path.join(base, 'memory', 'index.md'), '# Cursor memory sentinel\n');
      ...
      const payload = hookPayload('compact', base);
      expect(payload.user_message).toContain('Cursor memory sentinel');
