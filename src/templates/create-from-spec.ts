    ...
    try {
      ...
      if (preparedSnapshot.packages.apt.length > 0 || preparedSnapshot.packages.npm.length > 0) {
        if (getSessionDriver().capabilities().imageBuild) {
          ...
          // No per-agent-group image on this driver: the session runtime is an
          // immutable image the bake published. The declaration stays recorded
          // above (ncl and Governance keep reporting it); what changes is where
          // it is satisfied — at COMPOSE time, by the recipe baking the union of
          // the composed templates' packages into that image, with a test that
          // goes red when a template declares one the image lacks
          // (skills/agent-image-packages). Refusing here instead is what the
          // first governed provision on nancy-v3 did, naming neither the
          // package nor the template.
          log.warn('template packages are declared, not built, on this runtime driver: the agent image is expected to carry them', {
            template: spec.template.ref,
            apt: preparedSnapshot.packages.apt,
            npm: preparedSnapshot.packages.npm,
          });
        }
      }
      return group;
