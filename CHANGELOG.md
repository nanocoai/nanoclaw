...
- [BREAKING] **iMessage unified into one `imessage` channel with two pluggable backends (`/add-imessage`).** Local (this Mac's `chat.db` via the Chat SDK bridge over `chat-adapter-imessage`) or Hosted (native [Photon](https://photon.codes) via `spectrum-ts`, an in-process gRPC stream — no Mac relay, no webhook — provisioned by the device-login wizard `scripts/photon-setup.ts`). The backend is chosen at install (or forced via `IMESSAGE_BACKEND=local|hosted`); both `/setup` and `/add-imessage` ask Local vs Hosted and install only the chosen backend's package. The legacy Chat-SDK "remote" mode (`IMESSAGE_SERVER_URL`/`IMESSAGE_API_KEY`) and the separate `imessage-cloud` channel + `/add-imessage-cloud` skill are **removed** — everything is the `imessage` channel now (user ids `imessage:<phone-or-handle>`). Hosted supports both directions, markdown, attachments, tapback reactions, read receipts, typing, and `ask_user_question` via slash-command replies. Hosted setup includes one manual dashboard step: numbers only enter Photon routing once the dashboard's Add-user invite is accepted (the API cannot trigger it), so the wizard guides that step and waits for the opt-in before reporting success. See [docs/imessage.md](docs/imessage.md).
