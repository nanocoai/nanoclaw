 ...
 *
 * An install-overlay (e.g. a downstream installer) that needs to add per-spawn host-side
 ...
 * `{ mounts: [], env: {} }`, so with no registrant containers spawn with only the
 ...
 * contributors — a contributor is downstream code but is treated as untrusted by the
 * core merge so a future buggy/hostile contributor can't punch through a security
 * guard):
 *
 *   - RESERVED containerPaths (read-only-mount invariant): a contributor MUST NOT mount onto
 ...
 *     in container-runner — a contributor remounting it RW would let a
...
export interface ContainerContributorContext {
  ...
  sessionDir: string;
  /** `DATA_DIR` — root for host-owned shared DBs (e.g. extra, embeddings). */
  dataDir: string;
  /** Agent group folder (e.g. `my-agent`) — used for downstream routing-key resolution. */
...
/** containerPaths a contributor may never mount onto. See module doc (read-only-mount invariant). */
 ...
 * when no contributor is registered.
 */
