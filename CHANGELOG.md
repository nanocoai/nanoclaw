...
- **The community portal records the Echo image that actually landed.** Setup now fetches (or builds) the sandbox image before it tells the portal the Echo step is complete, and signing in no longer decides the image by itself: a browser step that ends without an answer leaves the question open for the next run instead of quietly pulling. See [docs/community-portal.md](docs/community-portal.md).
