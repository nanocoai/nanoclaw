 ...
 * to run extra carry-over work AFTER that seed loop — e.g. a downstream overlay
 * carrying a v1 `registered_groups` flag over to a v2 `messaging_groups`
 * column. That logic is overlay-private, so core
 ...
 * throws on duplicate, runs in registration order. With no registrant there are
