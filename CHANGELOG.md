...
- **Managed fleets can bypass the startup upgrade tripwire with `NANOCLAW_SKIP_UPGRADE_TRIPWIRE=1`.** Self-managed deployments that bake NanoClaw into an immutable image install through their own flow and skip the sanctioned `service` setup step (which stamps the upgrade marker), so the marker is never written and every boot would otherwise trip. The opt-out is explicit and logged at `warn`; default behaviour is unchanged.
