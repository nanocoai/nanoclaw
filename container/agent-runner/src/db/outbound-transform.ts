 ...
 * through. Core ships an IDENTITY transform so upstream builds
 * and behaves unchanged with zero downstream modules present. An install
 * overlay (e.g. a routing-aware reply gate) may replace it by calling
 ...
 * `writeMessageOut` so a routing-keyed reply can never silently fall back
