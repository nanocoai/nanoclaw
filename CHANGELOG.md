...
- **A resumed conversation is now told when its memory went stale.** Memory is injected into context only when a provider opens a new context window (`startup`, `clear`, `compact`) — never on `resume`. Because a session's continuation survives container kills, a conversation could resume across any number of restarts (including the 30-minute idle sweep and `ncl groups restart`) while its in-context copy of `memory/` aged silently: teach an agent something in one session, and its other sessions kept answering from the older copy indefinitely. A resumed turn now opens with a `<memory_changed>` block naming the files that moved since its last turn, so the agent re-reads them instead of trusting what it already has. Nothing is emitted when nothing changed, and the agent's own edits during a turn are never reported back to it. Fires once per provider query, so a warm stream picks up an edit at its next query rather than mid-turn.
