...
export async function savePortalAccount(client: SetupClient): Promise<void> {
  ...
  await writePrivate(registryAuthPath(), {
    version: 1,
    broker_url: record.api,
...
function portalClient(): SetupClient {
...
export async function offerPortalReminder(stage: 'echo' | 'slack', enable: () => Promise<void>): Promise<boolean> {
  const client = await portalClient().initialize();
  let consent = false;
  try {
...
export async function beginPortal(
  ...
  try {
    ...
    if (!(await client.available(stage))) {
      await client.stop();
      return null;
    }
    if (
      ...
      p.log.info(
        `${stage === 'perks' ? 'Partner perks' : stage[0].toUpperCase() + stage.slice(1)} already enabled. Continuing setup.`,
      );
      return client;
    }
    const labels = {
      echo: 'Echo’s hardened agent image',
      slack: 'Slack for your agent',
      perks: 'partner perks',
      tavily: 'Tavily web search',
      dial: 'Dial, a phone number for your agent',
    };
    const consent =
      browserConsent ||
      (await p.confirm({
...
export async function runImagePortal(
  ...
  try {
    ...
    if (result.status !== 'skipped' || client.local.registryAccount) {
      await savePortalAccount(client);
...
export async function runSlackPortal(
  ...
  try {
    const setup = await client.wait(),
      choice = setup.choice;
    if (setup.status !== 'skipped' || client.local.registryAccount) {
      await savePortalAccount(client);
      await client.reconcile();
    }
