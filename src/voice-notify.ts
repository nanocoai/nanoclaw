...
- 链接绝不念 URL，按语义转一句话：GitHub PR 说"某某仓库的几号PR"，看不出指向的说"详情有链接，看文字版"
...
${V2_COMMON_RULES}`,
  ...
  navigate: `你把一段多章节的 AI 回复改写成"导航式"语音播报。用户听完要能判断：这篇讲了什么、该看哪一段、有没有要他拍板的。

策略：结论 + 章节地图 + 行动点，控制在 200 字以内。
- 第一句必须是全文结论（一句话，不展开）
- 然后列出章节结构："正文 N 部分——第一部分讲什么、第二部分讲什么"（只到标题级，不展开内容）
- 如果某一节需要用户决策/拍板，必须点出来："第几部分需要你定"
- 最后加一句"详情看屏幕"或"详情看文字版"
- 不要展开任何章节的具体内容、不要复述数据/代码
${V2_COMMON_RULES}`,

  digest: `你把一段较长的 AI 回复改写成口语化的语音播报版本。原文可能是列表、多步骤操作、长段分析。

策略：结论 + 关键几条 + 总数提示，控制在 150 字以内。
- 第一句必须是结论或最终结果
- 如果原文有多个要点/步骤，只说最关键的 2-3 条，然后说"一共几条，其余看文字版"
- 不要逐条复述所有步骤
...
- 不要说"代码如下"、"具体代码请看文字版"这种过渡——直接说结论，代码的存在对听者透明
${V2_COMMON_RULES}`,
...
- 如果需要用户看表格做决定，说"有个对比表需要你看一下，详情看文字版"
${V2_COMMON_RULES}`,
};
...
async function summarizeForSpeech(
  ...
  if (summaryV2) {
    ...
    logger.info(
      { category, chars: text.length },
      '[voice-notify] v2 摘要分流',
    );
  } else {
