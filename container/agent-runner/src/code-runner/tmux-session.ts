...
export interface TmuxSessionOptions {
   ...
   * the resume fallback: a boot-time `--continue` the CLI cannot load
...
export class TmuxSession {
  ...
  private lives = 0;
  /** Args of the CURRENT and future lives — swapped once by the resume fallback. */
  ...
  private async checkLife(): Promise<void> {
    ...
    try {
      ...
      this.scheduleRespawn(async () => {
        ...
        // after the resume fallback swapped the args, the new command must ride
  ...
  private scheduleRespawn(action: () => Promise<void>): void {
    ...
    // resume fallback: a FIRST life that never reached a healthy run is a boot
    ...
    if (!healthy && this.lives === 1 && this.opts.fallbackArgs && !this.fellBack) {
      ...
      console.error(
        '[code-runner] first session life died before its healthy-run window — falling back to fresh-boot args',
      );
    }
