...
- **Channel/provider install skills** — copy the relevant module(s) in from the `channels` or `providers` branch, wire imports, install pinned deps (e.g. `/add-discord`, `/add-slack`, `/add-whatsapp`, `/add-opencode`). User-invoked only (`disable-model-invocation: true`; `agents/openai.yaml` with `policy.allow_implicit_invocation: false` for Codex), so they are never in your context — do not paraphrase their steps. Install one with `bin/ncl skills apply <name>` (`plan` previews it; `--restart` reloads the service; host down: `pnpm exec tsx scripts/skill-headless.ts apply <name>`). If the engine refuses it as prose-only, ask the user to run `/<name>`.
