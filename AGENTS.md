...
2. Tell the user, verbatim: *"This is the v2 rewrite — it can't be merged into your existing install. Exit Codex (or open a separate terminal) and run `bash migrate-v2.sh` from the shell."*
3. Wait for the user to confirm before doing anything else. Do not run the migration script yourself — it requires an interactive terminal and cannot be run from within Codex.
...
| `migrate-v2.sh` + `setup/migrate-v2/` | v1→v2 migration. Standalone script: `bash migrate-v2.sh`. Seeds DB, copies groups/sessions, installs channels, builds container, offers service switchover, then hands off to `/migrate-from-v1` skill for owner setup and AGENTS.md cleanup. See [docs/migration-dev.md](docs/migration-dev.md). |
...
| Value | Behavior |
|-------|----------|
| `disabled` | Agent never learns about ncl (instructions excluded from AGENTS.md). Host dispatch rejects any `cli_request`. |
...
Key files: `src/db/container-configs.ts`, `src/container-config.ts`, `src/cli/dispatch.ts` (scope enforcement), `src/Codex-md-compose.ts` (instructions exclusion).
...
Show the output and wait for approval. Installation-specific files (group files, .Codex/settings.json, local configs) should not be included.
...
| [docs/setup-flow.md](docs/setup-flow.md) | Setup logging, step contract, output levels, AI-coding-CLI picker, credential mode, Keychain pre-flight |
| [docs/setup-wiring.md](docs/setup-wiring.md) | What's wired, what's open in the setup flow |
| [docs/architecture-diagram.md](docs/architecture-diagram.md) | Diagram version of the architecture |
| [docs/build-and-runtime.md](docs/build-and-runtime.md) | Runtime split (Node host + Bun container), lockfiles, image build surface, CI, key invariants |
| [docs/credentials.md](docs/credentials.md) | Credential abstraction — gateway-injected vs native bundle, resolver hook |
