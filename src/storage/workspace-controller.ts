...
/** What `reconcile()` already listed, handed down instead of re-fetched. */
type ReconcileSnapshot = { mutations: number; lease: Json; pods: Json[]; reservations: Json[] };

const GROUP_LEASE_LABEL = 'nanoco.ai/workspace-group-lease';
const RESERVATION_LABEL = 'nanoco.ai/workspace-reservation';
/**
 * How long a group may sit idle before its Custodian is stopped.
 *
 * Stopping it reclaims ~25m CPU and ~70MiB — and costs the NEXT message about
 * seven seconds, because the session pod cannot be created until a Custodian
 * generation is Ready again. Measured on nancy-v3: custodian 0 → Ready 7s,
 * session pod 0 → Ready 8s, and the two are serialized, so a cold start pays
 * both before the agent has done anything.
 *
 * `NANOCO_WORKSPACE_CUSTODIAN_IDLE_MS=0` keeps every Custodian resident, which
 * is the right trade wherever people are waiting on replies: a fleet of 26
 * groups costs roughly 650m CPU and 1.8GiB to remove seven seconds from every
 * cold start. The default is unchanged for deployments that would rather have
 * the memory back.
 */
const IDLE_MS = (() => {
  const raw = process.env.NANOCO_WORKSPACE_CUSTODIAN_IDLE_MS;
...
export class WorkspaceController {
  ...
  /**
   * Bumped by every entrypoint that writes workspace state. `reconcile()`
   * stamps its list snapshot with the value it saw, so `#reconcileGroup` —
   * which runs later, behind `#serialized` — can tell whether that snapshot
   * still describes the cluster it is about to act on.
   */
  #mutations = 0;
  ...
  async reconcile(): Promise<void> {
    ...
    for (const pod of pods) {
      ...
      if (!groupId || !sessionId) continue;
      // A live Pod supersedes its own reservation, so the snapshot handed down
      // below has to forget what this loop just deleted: a group still read as
      // reserved takes the active branch and skips its final checkpoint.
      const name = reservationName(groupId, sessionId);
      released.add(name);
      await this.#kube.delete('lease', name, this.#namespace);
    }
    ...
    const live = reservations.filter((candidate) => !released.has(candidate.metadata?.name));
    for (const lease of groupLeases) {
      const groupId = lease.metadata?.annotations?.[WORKSPACE_GROUP_ANNOTATION];
      if (!groupId) continue;
      await this.#serialized(groupId, () => this.#reconcileGroup(groupId, now, { mutations, lease, pods, reservations: live }));
    }
  }
  ...
  async #ensure(input: EnsureRequest): Promise<WorkspaceAssignment> {
    ...
    if (podReady(current) && lease?.metadata?.annotations?.[CUSTODIAN_ATTEMPTS_ANNOTATION] &&
      ...
      // its backoff from zero rather than inheriting an old streak. This runs
      // BEFORE the image-upgrade branch below, which nulls `current` — read
      // after it, a healthy group being upgraded looks like a failing one and
      // is served a backoff it never earned.
      await this.#annotateLease(lease, { [CUSTODIAN_ATTEMPTS_ANNOTATION]: '0' });
      // #annotateLease patches the API, not this object; the backoff below
      // reads the local view, so it has to see what was just persisted.
      lease.metadata.annotations[CUSTODIAN_ATTEMPTS_ANNOTATION] = '0';
    }
  ...
  async #reconcileGroup(groupId: string, now: number, snapshot: ReconcileSnapshot): Promise<void> {
    // `reconcile()` already listed every workspace Pod, every reservation and
    // every group Lease, and this re-fetched strict subsets of exactly that
    // data: four more `kubectl` subprocesses per group per tick, which is 98%
    // of this Pod's CPU. Reuse the snapshot — but only while it is still
    // lock-consistent. `#serialized` runs this AFTER any queued `ensure()`,
    // and a snapshot taken before one is missing precisely the reservation and
    // dirty flag that stop an idle sweep from reaping a Custodian that was just
    // handed to a starting session, so a raced snapshot falls back to the fetch.
    const id = labelId(groupId);
    const [lease, groupPods, reservations]: [Json | null, Json[], Json[]] = snapshot.mutations === this.#mutations
      ? [
        snapshot.lease,
        snapshot.pods.filter((pod) => pod.metadata?.labels?.[WORKSPACE_GROUP_LABEL] === id),
        snapshot.reservations.filter((candidate) => candidate.metadata?.labels?.[WORKSPACE_GROUP_LABEL] === id),
      ]
      : await Promise.all([
        this.#kube.get('lease', groupLeaseName(groupId), this.#namespace),
        this.#kube.list('pods', this.#agentsNamespace, `${WORKSPACE_GROUP_LABEL}=${id}`),
        this.#kube.list('leases.coordination.k8s.io', this.#namespace, `${RESERVATION_LABEL}=true,${WORKSPACE_GROUP_LABEL}=${id}`),
      ]);
    if (!lease) return;
    ...
    } else if (IDLE_MS > 0 && now - idle >= IDLE_MS) {
      // IDLE_MS === 0 keeps the Custodian resident: the group stays warm and
      // the next message skips the seven seconds a fresh generation costs.
...
/**
 * The reconcile cadence IS this Pod's CPU profile: every tick shells out one
 * `kubectl` subprocess per API call, and those subprocesses — not the Node
 * process around them — are 98% of what the Controller burns. The default stays
 * 5s because nothing measured says a slower loop is safe, but the period
 * becomes an operator knob so a crowded node can be paced without a new image.
 */
function reconcileIntervalMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NANOCO_WORKSPACE_RECONCILE_MS?.trim();
  if (!raw) return RECONCILE_MS;
  const parsed = Number(raw);
  if (!Number.isSafeInteger(parsed) || parsed < 250 || parsed > 3_600_000) throw new Error('invalid workspace reconcile interval');
  return parsed;
}

/**
 * The Controller carries no logging framework and said nothing at all on the
 * success path, so a healthy reconciler and a wedged one looked identical from
 * outside. One line per tick would be 17k lines a day at the default period, so
 * collapse the loop into a single counted line: immediately on the first tick,
 * so a starting Pod proves itself, and once every HEARTBEAT_MS after that.
 */
function reconcileHeartbeat(): (error?: Error) => void {
  let ok = 0; let failed = 0; let since = Date.now(); let due = 0;
  return (error?: Error): void => {
    if (error) failed += 1; else ok += 1;
    const at = Date.now();
    if (at < due) return;
    process.stdout.write(`workspace reconcile: ${ok} ok, ${failed} failed in ${Math.max(1, Math.round((at - since) / 1000))}s\n`);
    ok = 0; failed = 0; since = at; due = at + HEARTBEAT_MS;
  };
}
...
export async function startEmbeddedWorkspaceController(
  ...
  const period = reconcileIntervalMs(env);
  const heartbeat = reconcileHeartbeat();
  let stopped = false;
  ...
  const reconcile = (): void => {
    ...
    controller.reconcile()
      .then(() => heartbeat())
      .catch((error) => {
        heartbeat(error as Error);
        process.stderr.write(`workspace reconcile failed: ${(error as Error).message}\n`);
      })
      .finally(() => {
        if (stopped) return;
        timer = setTimeout(reconcile, period);
        timer.unref();
      });
  };
  reconcile();
...
async function main(): Promise<void> {
  ...
  const period = reconcileIntervalMs(process.env);
  const heartbeat = reconcileHeartbeat();
  const reconcile = (): void => {
    controller.reconcile()
      .then(() => heartbeat())
      ...
      .finally(() => setTimeout(reconcile, period).unref());
  };
  reconcile();
}
