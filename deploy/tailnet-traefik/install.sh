...
usage: install.sh --tailnet-host HOST --system-namespace NAME \
  --mocks-namespace NAME [--governance-port PORT] [--governance-consent-port PORT] \
...
while (($#)); do
  case "$1" in
    ...
    --governance-consent-port) governance_consent_port="${2:-}"; shift 2 ;;
...
for port in \
  ...
  "$governance_port" "$governance_consent_port" "$backlot_control_port" "$backlot_slack_port" \
...
$(edge_hostname governance)
  rules:
    - matches:
        - path: {type: Exact, value: /oauth/callback}
      backendRefs: [{name: governance-consent, port: $governance_consent_port}]
  ...
  ingress:
    - from:
        - namespaceSelector: {matchLabels: {kubernetes.io/metadata.name: kube-system}}
          podSelector: {matchLabels: {app.kubernetes.io/name: nanoco-tailnet-traefik}}
      ports:
        - {protocol: TCP, port: $governance_port}
        - {protocol: TCP, port: $governance_consent_port}
    - from:
...
expected_routes=1
[[ -z "$edge_domain" ]] || expected_routes=$((expected_routes + 1))
...
for _ in {1..60}; do
  ...
  if jq -e --argjson expected "$expected_route_parents" --argjson expectedRoutes "$expected_routes" '
...
# Tailscale is transport only. Prove each exact-host origin this deployment
# composed reaches its product-owned route without making Governance or
# Traefik consume a Tailscale-specific identity header.
route_probes=("governance|$governance_listen_port|/health")
if [[ -n "$mocks_namespace" ]]; then
  route_probes+=(
...
for route_probe in "${route_probes[@]}"; do
...
if [[ -n "$edge_domain" ]]; then
  edge_route_probes=("governance|/health")
  if [[ -n "$mocks_namespace" ]]; then
    edge_route_probes+=(
      "control|/healthz"
      "slack|/"
  ...
  for route_probe in "${edge_route_probes[@]}"; do
...
remove_tailscale_serve() {
  local output
  if output="$(sudo -n tailscale serve --yes --https="$1" off 2>&1)"; then
    return 0
  fi
  [[ "$output" == *'handler does not exist'* ]] && return 0
  printf '%s\n' "$output" >&2
  return 1
}
...
if [[ -n "$mocks_namespace" ]]; then
  ...
  for public_port in \
    "$backlot_control_port" "$backlot_slack_port" "$backlot_teams_port" \
    "$backlot_okta_port" "$backlot_google_port" "$backlot_salesforce_port"; do
    remove_tailscale_serve "$public_port"
  done
fi
...
expected_loopback_ports=("$governance_listen_port" "$child_chat_listen_port")
if [[ -n "$mocks_namespace" ]]; then
  expected_loopback_ports+=(
    "$control_listen_port" "$slack_listen_port" "$teams_listen_port"
    "$okta_listen_port" "$google_listen_port" "$salesforce_listen_port"
  )
fi
for loopback_port in "${expected_loopback_ports[@]}"; do
