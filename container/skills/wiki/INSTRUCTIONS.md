...
3. 创建或更新 team_wiki 页面：
   - **综合进已有相关页的对应小节并回收旧态**（优先），或新建主题页
   - "回收旧态"= 被新进展取代的旧描述要删除或更新，不能新旧并存让读者猜
   - 顶部「当前状态」是状态唯一出处，新进展只更新这一处
   - 禁止页内行号/节号引用（用小标题锚点替代）
   - 详见 `team_wiki/CONTRIBUTING.md` 写作规范
   - 相关实体页（项目、人员、技术）/ 概念页
...
**重要**：多个资料必须逐一处理，不要批量。"综合进已有页的对应小节并回收旧态"优先于新建孤立碎片。
