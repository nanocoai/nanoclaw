...
- **Dial joins the setup wizard: a real phone number for SMS and AI voice calls.** The channel picker offers *Yes, connect Dial*; `/add-dial` installs the native adapter from the `channels` branch, signs you in to [Dial](https://getdial.ai) with an email code, pins the CLI path into `.env` (`DIAL_CLI_PATH`), asks whether the line is owner-only or public, and pairs your phone with a 6-digit SMS code. `/add-dial-number` adds further lines to one install. `/add-dial-tool` gives chosen agents the `dial` CLI inside the sandbox, with the key in the OneCLI vault and every other agent blocked by a per-agent rule; re-run it to change who may use it. Outbound SMS to US numbers needs 10DLC registration on the Dial side; the adapter reports carrier bounces to the sending agent as a system notice instead of marking them delivered.
