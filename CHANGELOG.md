...
- **Typing indicator no longer drops during a single long tool call.** The refresh in `src/modules/typing/index.ts` previously gated only on the container's `.heartbeat` file staying fresh (touched within 6s), which only ticks on each SDK event — a lone slow Bash command, network fetch, or MCP call with no interim event could go quiet for minutes with typing stopped well before the agent finished, looking stuck. Now also checks `container_state` (already written on `PreToolUse`/`PostToolUse` for host-sweep's own stuck-detection tolerance) and keeps typing alive whenever a tool is currently in flight, regardless of heartbeat staleness. Host-only change, no container image rebuild required.
