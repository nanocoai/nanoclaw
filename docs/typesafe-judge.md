...
`api.typesafe.ai` credential with the install's gateway. The agent gets a
...
- **An upstream defect never becomes an `act`.** An answer must match its
  ...
  legend naming exactly those levels. Anything else is exit 3.
...
- **Retries**: 429 and 529 are retried with exponential backoff that waits at
  least `Retry-After` (capped at 60 s).
...
`NANOCLAW_GATEWAY_PROVIDER` (environment first, then `.env`) and drives the
matching gateway:
...
- **OneCLI** (default): the operator stores the key themselves, either
  ...
  read; `all`-mode agents need nothing), and creates one rate-limit rule,
  "TypeSafe: spend ceiling" (600 requests an hour; OneCLI counts it per
  agent, so it caps each agent rather than the host; edit the number in the
  dashboard). A same-named rule that is disabled, scoped, or on another host
  fails the step instead of passing as protection.
