...
export function buildAddAgentDestinationCommand(agentGroupId: string, localName: string, targetId: string): string {
  return `ncl destinations add --agent-group-id ${agentGroupId} --local-name ${localName} --target-type agent --target-id ${targetId}`;
}

/** Display name for chat text, or a lowercase role fallback ("an admin"). */
export async function approverLabel(userId: string): Promise<string> {
