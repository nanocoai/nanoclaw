 ...
 * origin, CONNECT tunnelling of `node:https`, NO_PROXY, idempotence, restore.
 */
import fs from 'fs';
...
describe('Cursor traffic through the credential gateway proxy', () => {
  ...
  it('refuses the HTTP/2 transport when the server forces it, without a direct connection', async () => {
    ...
    const texts = summary.events.flatMap((event) =>
      [event.message, event.text, event.error].filter((value): value is string => typeof value === 'string'),
    );
...
describe('installHttp2ProxyGuard (client-side refusal, in-process)', () => {
  it('installs nothing when no proxy is configured', () => {
    const before = { http2: http2.connect, tls: tls.connect, https: https.Agent.prototype.createConnection };
    ...
    expect(https.Agent.prototype.createConnection).toBe(before.https);
  });
  ...
  it('refuses a TLS dial with ALPN h2 to any origin but the proxy and passes every other dial through', () => {
    ...
    // Non-h2 TLS to the proxy itself is not a bypass.
  ...
  // Bun 1.3 has no https.Agent.createConnection; its node:https client already
  // honors HTTPS_PROXY, which the child-process cases cover. Bun 1.4's method
  // dials the origin, and this is the wrap that sends it through CONNECT.
  it.skipIf(typeof https.Agent.prototype.createConnection !== 'function')(
    'tunnels node:https through the proxy instead of dialing the origin',
    async () => {
      ...
      const proxy = net.createServer((client) => {
        ...
        const onData = (chunk: Buffer): void => {
          ...
          if (!buf.includes('\r\n\r\n')) return;
          client.off('data', onData);
          connects.push(buf.slice(0, buf.indexOf('\r\n')));
      ...
      restore = installHttp2ProxyGuard({ HTTPS_PROXY: `http://127.0.0.1:${port}` });
      await new Promise<void>((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error('timed out waiting for the proxy tunnel')), 5_000);
        const done = (): void => {
          clearTimeout(timer);
          resolve();
        };
        const req = https.get('https://api2.cursor.sh/v1/models');
        req.on('error', done);
        req.on('response', (res) => {
          res.resume();
          done();
      ...
      proxy.close();
      expect(connects).toEqual(['CONNECT api2.cursor.sh:443 HTTP/1.1']);
    },
  );

  it.skipIf(typeof https.Agent.prototype.createConnection !== 'function')(
    'sends Proxy-Authorization when the proxy URL carries userinfo',
    async () => {
      ...
      const proxy = net.createServer((client) => {
        ...
        const onData = (chunk: Buffer): void => {
          ...
          if (!buf.includes('\r\n\r\n')) return;
          client.off('data', onData);
          headers.push(buf.slice(0, buf.indexOf('\r\n\r\n')));
          client.end('HTTP/1.1 200 Connection Established\r\n\r\n');
        };
        client.on('data', onData);
      });
      ...
      const basic = (user: string, password: string): string =>
        `Proxy-Authorization: Basic ${Buffer.from(`${user}:${password}`, 'utf8').toString('base64')}`;

      restore = installHttp2ProxyGuard({ HTTPS_PROXY: `http://proxyuser:s3cret@127.0.0.1:${port}` });
      await request();
      restore();
      restore = installHttp2ProxyGuard({ HTTPS_PROXY: `http://proxyuser:p%40ss@127.0.0.1:${port}` });
      await request();
      restore();
      restore = installHttp2ProxyGuard({ HTTPS_PROXY: `http://127.0.0.1:${port}` });
      await request();
      ...
      expect(headers[0]).toContain('CONNECT api2.cursor.sh:443 HTTP/1.1');
      expect(headers[0]).toContain(basic('proxyuser', 's3cret'));
      expect(headers[1]).toContain(basic('proxyuser', 'p@ss'));
      expect(headers[2]).not.toContain('Proxy-Authorization');
    },
  );

  it('is idempotent and restores the originals', () => {
    const originals = { http2: http2.connect, tls: tls.connect, https: https.Agent.prototype.createConnection };
    restore = installHttp2ProxyGuard(PROXY_ENV);
    const guarded = { http2: http2.connect, tls: tls.connect, https: https.Agent.prototype.createConnection };
    expect(guarded.http2).not.toBe(originals.http2);
    expect(guarded.tls).not.toBe(originals.tls);
    if (typeof originals.https === 'function') expect(guarded.https).not.toBe(originals.https);

    const again = installHttp2ProxyGuard(PROXY_ENV);
    expect(http2.connect).toBe(guarded.http2);
    expect(tls.connect).toBe(guarded.tls);
    if (typeof originals.https === 'function') expect(https.Agent.prototype.createConnection).toBe(guarded.https);
    again?.();
    expect(http2.connect).toBe(guarded.http2);
    expect(tls.connect).toBe(guarded.tls);
    if (typeof originals.https === 'function') expect(https.Agent.prototype.createConnection).toBe(guarded.https);
    ...
    expect(https.Agent.prototype.createConnection).toBe(originals.https);
  });
});
