...
export function answerMatches(question: Question, v: unknown): v is Answer {
  ...
  if (v.type === 'score') {
    ...
    // The legend must name exactly the offered levels: gate.level reads from it.
    if (!isRecord(v.legend) || Object.keys(v.legend).length !== levels || !keys.every((k) => typeof (v.legend as Record<string, unknown>)[k] === 'string')) return false;
