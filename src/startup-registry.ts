/**
 * Startup-hook registry (ADR 0006 startup contract) — INERT with no registrant.
 *
 * The composition root (`src/index.ts`) is a thin orchestrator. Install-overlays
 * (e.g. a downstream installer) that need to run extra boot steps register a startup hook
 ...
 *                       (e.g. a downstream service session whose outbound.db the
 *                       sweep drains).
 ...
 * both phases, so upstream boots with only the default orchestration. An
 ...
 *     invariants the rest of boot depends on (a downstream service session must
 *     exist before the delivery polls start, or service-originated rows have no
...
export interface StartupContext {
  /** `DATA_DIR` — root for host-owned shared DBs (e.g. v2.db, extra, embeddings). */
  dataDir: string;
}
 ...
 * order. No-op with no registrant (no hooks). A non-critical hook that throws is
