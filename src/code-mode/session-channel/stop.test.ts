...
describe('isStopEvent', () => {
  it('a channel-wide stopped event is a stop; a thread stop or another type is not', () => {
    expect(isStopEvent({ type: 'code_channel.stopped' })).toBe(true);
    expect(isStopEvent({ type: 'code_channel.stopped', threadTs: '1.2' })).toBe(false);
    expect(isStopEvent({ type: 'code_channel.something_else' })).toBe(false);
  });
});
