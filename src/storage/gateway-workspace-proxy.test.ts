...
const upstream = vi.fn(async (input: string, init: RequestInit = {}) => {
  ...
  if (url.hostname.startsWith('s3.')) {
    ...
    if (prefix) {
      return new Response(
        `<ListBucketResult><Prefix>${prefix}</Prefix><KeyCount>3</KeyCount>` +
        `<Contents><Key>${prefix}group-a/config</Key><ETag>&quot;a&quot;</ETag></Contents>` +
        `<Contents><Key>${prefix}group-b/config</Key><ETag>&quot;b&quot;</ETag></Contents>` +
        `<Contents><Key>${prefix}group-a/keys/de90646f25</Key><ETag>&quot;k&quot;</ETag></Contents>` +
        `</ListBucketResult>`,
...
describe('Gateway workspace loopback proxy', () => {
  test('widens one ListObjects request to the trusted environment root and filters siblings', async () => {
    ...
    expect(xml).toContain(`${options.scopePrefix}/group-a/keys/de90646f25`);
  ...
  test('a delimiter list is forwarded RECURSIVE and regrouped client-side — restic finds its keys', async () => {
    // nancy-v3, custodian g12 tap: restic 0.18.1 lists `keys/` WITH a
    // delimiter. The proxy widened the prefix to the scope root but forwarded
    // the delimiter, so S3 folded the requested subtree into a CommonPrefix,
    // the Contents filter matched nothing, and restic reported "Fatal: wrong
    // password or no key found" against a byte-perfect repository — eleven
    // custodian generations of it.
    proxy = await startGatewayWorkspaceProxy(options);
    const requested = `${options.scopePrefix}/group-a/keys/`;
    const response = await fetch(
      `${proxy.s3Endpoint}/${options.bucket}?list-type=2&delimiter=%2F&prefix=${encodeURIComponent(requested)}`,
      { headers: syntheticHeaders() },
    );
    expect(response.status).toBe(200);
    const xml = await response.text();
    ...
    // The response echoes the CLIENT's request, not the widened root.
    expect(xml).toContain(`<Prefix>${requested}</Prefix>`);
    expect(xml).toContain('<Delimiter>/</Delimiter>');
    const [target] = upstream.mock.calls[0]!;
    const forwarded = new URL(String(target)).searchParams;
    expect(forwarded.get('prefix')).toBe(`${options.scopePrefix}/`);
    expect(forwarded.has('delimiter')).toBe(false);
  });

  test('a delimiter list at a parent groups children into CommonPrefixes', async () => {
    proxy = await startGatewayWorkspaceProxy(options);
    const requested = `${options.scopePrefix}/`;
    const response = await fetch(
      `${proxy.s3Endpoint}/${options.bucket}?list-type=2&delimiter=%2F&prefix=${encodeURIComponent(requested)}`,
      { headers: syntheticHeaders() },
    );
    expect(response.status).toBe(200);
    const xml = await response.text();
    expect(xml).toContain(`<CommonPrefixes><Prefix>${requested}group-a/</Prefix></CommonPrefixes>`);
    expect(xml).toContain(`<CommonPrefixes><Prefix>${requested}group-b/</Prefix></CommonPrefixes>`);
    expect(xml).not.toMatch(/<Contents>/);
    expect(xml).toContain('<KeyCount>2</KeyCount>');
  });
