...
interface ProxyPolicy {
  ...
  /** Basic credentials from the proxy URL, when it has them. */
...
/** URL userinfo is percent-encoded. Decode it so Basic matches what fetch sends. */
function proxyAuthorizationHeader(url: URL): string | null {
  ...
  const token = Buffer.from(`${decode(url.username)}:${decode(url.password)}`, 'utf8').toString('base64');
  return `Basic ${token}`;
}
...
function proxyPolicy(env: NodeJS.ProcessEnv): ProxyPolicy | null {
  ...
  try {
    ...
    return {
      ...
      proxyPort: url.port || defaultPort(url.protocol),
      proxyAuthorization: proxyAuthorizationHeader(url),
      noProxy,
    };
  } catch {
    return { proxyHost: null, proxyPort: null, proxyAuthorization: null, noProxy };
  }
}
...
/**
 * Close the Cursor network paths the credential gateway would not see.
 *
 * Bun's `node:http2` client ignores HTTPS_PROXY, so an HTTP/2 Connect
 * transport would carry the agent's runtime token straight to Cursor. While a
 * proxy is configured, any HTTP/2 session to an origin other than the proxy
 * itself (or a NO_PROXY host, which the operator has excluded from the proxy
 * on purpose) throws `http2ProxyBypassError` instead of dialing. That path is
 * not tunnelled: the refusal is the whole behaviour, and the SDK surfaces it
 * as a run error.
 *
 * Bun 1.4's `node:https` agent has the same hole unless `NODE_USE_ENV_PROXY`
 * was set when the process started, which the container does not do.
 * `@connectrpc/connect-node` sends its HTTP/1.1 RPCs through that agent, and
 * `https.Agent.createConnection` then calls `tls.connect` straight at the
 * origin. While a proxy is configured, that one method is wrapped to open a
 * CONNECT tunnel and start TLS on the tunnel socket, so those RPCs reach the
 * gateway instead of Cursor's address.
 *
 * Three hooks, because of how Bun binds builtins:
 *
 ...
 *     `http2.connect`. A handshake over a socket the caller already opened is
 *     left alone: that is the HTTP/1.1 tunnel finishing, not a new TCP dial.
 *   - `https.Agent.createConnection`. This is the Bun 1.4 `node:https` dial
 *     described above. It returns no socket of its own; the agent waits on
 *     the callback, which fires with a TLS socket only after CONNECT
 *     succeeds.
 *
...
export function installHttp2ProxyGuard(env: NodeJS.ProcessEnv = process.env): (() => void) | null {
  ...
  if (!originalTlsConnect[HTTP2_PROXY_GUARD]) {
    const guardedTlsConnect = function (this: unknown, ...args: unknown[]) {
      const options = args.find((arg) => arg !== null && typeof arg === 'object' && !Array.isArray(arg)) as
        ...
        | undefined;
      if (options?.socket) {
        return (originalTlsConnect as unknown as (...connectArgs: unknown[]) => tls.TLSSocket).apply(this, args);
      }
  ...
  // Bun 1.3's HTTPS agent has no `createConnection` and already honors
  // HTTPS_PROXY. Bun 1.4 added the method and, without NODE_USE_ENV_PROXY at
  // process start, it dials the origin. Wrap only the method that exists.
  const httpsAgent = https.Agent.prototype as {
    createConnection?: GuardMarked<typeof https.Agent.prototype.createConnection>;
  };
  const originalCreateConnection = httpsAgent.createConnection;
  if (originalCreateConnection && !originalCreateConnection[HTTP2_PROXY_GUARD]) {
    const guardedCreateConnection = function (this: https.Agent, ...args: unknown[]) {
      const parsed = httpsConnectionArgs(args);
      if (!parsed?.callback) return originalCreateConnection.apply(this, args as never);
      const target = dialTarget(parsed.options);
      if (permitsDial(policy, target.host, target.port)) return originalCreateConnection.apply(this, args as never);
      if (!policy.proxyHost || !policy.proxyPort) {
        parsed.callback(
          new Error(
            `Cursor SDK attempted an HTTPS connection to https://${target.host}:${target.port} that would bypass the configured proxy; refusing (see /add-cursor troubleshooting)`,
          ),
        );
        return undefined;
      }
      connectViaHttpProxy(policy, parsed.options, target, parsed.callback);
      return undefined;
    } as unknown as GuardMarked<typeof https.Agent.prototype.createConnection>;
    guardedCreateConnection[HTTP2_PROXY_GUARD] = true;
    httpsAgent.createConnection = guardedCreateConnection;
    restores.push(() => {
      httpsAgent.createConnection = originalCreateConnection;
    });
  }
...
interface HttpsConnectArgs {
  options: tls.ConnectionOptions & { hostname?: string };
  callback: (err: Error | null, socket?: tls.TLSSocket) => void;
}

function httpsConnectionArgs(args: unknown[]): HttpsConnectArgs | null {
  const last = args[args.length - 1];
  if (typeof last !== 'function') return null;
  const options = args.find((arg) => arg !== null && typeof arg === 'object' && !Array.isArray(arg)) as
    | (tls.ConnectionOptions & { hostname?: string })
    | undefined;
  if (!options) return null;
  return { options, callback: last as HttpsConnectArgs['callback'] };
}

function dialTarget(options: tls.ConnectionOptions & { hostname?: string }): { host: string; port: string } {
  const host = options.host || options.hostname || options.servername || 'localhost';
  const port = options.port == null ? '443' : String(options.port);
  return { host, port };
}

/**
 * Bun's HTTPS agent calls this instead of dialing. CONNECT to the proxy, then
 * hand the agent a TLS socket over that tunnel. Returning no socket is what
 * makes the agent wait: it only adopts the socket from the callback when
 * `createConnection` returns nothing (or a socket marked as waiting on a
 * tunnel). The callback fires once, after the proxy's `200`, with the TLS
 * handshake still ahead of it.
 */
function connectViaHttpProxy(
  ...
  const proxySocket = net.connect({ host: policy.proxyHost!, port: Number(policy.proxyPort) });
  let settled = false;
  ...
  proxySocket.on('data', function onData(chunk: Buffer) {
    ...
    const status = buffered.subarray(0, buffered.indexOf('\r\n')).toString('latin1');
    if (!/^HTTP\/1\.[01] 200\b/.test(status)) {
      fail(new Error(`Cursor SDK proxy tunnel to ${authority} failed: ${status}`));
      return;
    }
    if (settled) return;
    settled = true;
    ...
    const tlsOptions: tls.ConnectionOptions = {
      socket: proxySocket,
      servername: options.servername || target.host,
    };
    if (options.ALPNProtocols !== undefined) tlsOptions.ALPNProtocols = options.ALPNProtocols;
    if (options.rejectUnauthorized !== undefined) tlsOptions.rejectUnauthorized = options.rejectUnauthorized;
    ...
    if (options.session !== undefined) tlsOptions.session = options.session;
    if (options.checkServerIdentity !== undefined) tlsOptions.checkServerIdentity = options.checkServerIdentity;
    try {
      callback(null, tls.connect(tlsOptions));
    } catch (err) {
      proxySocket.destroy();
  ...
  proxySocket.on('connect', () => {
    // Bun's fetch sends the proxy URL's userinfo. This tunnel has to do the
    // same, or an authenticating proxy rejects the agent RPC.
    const lines = [`CONNECT ${authority} HTTP/1.1`, `Host: ${authority}`];
    if (policy.proxyAuthorization) lines.push(`Proxy-Authorization: ${policy.proxyAuthorization}`);
 ...
 *     HTTP/1.1, which is `node:https`. Bun 1.4's HTTPS agent dials the origin
 *     directly unless NODE_USE_ENV_PROXY was set at process start, so
 *     `installHttp2ProxyGuard` tunnels `https.Agent.createConnection` through
 *     the proxy before the TLS handshake.
...
/**
 * A failed turn. `error` is the channel notice on a core that has it;
 * older cores type the result without that field, so it is attached
 * without changing the declared event type. `text` stays model output.
 */
function failedResult(error: string, text: string | null = null): ProviderEvent {
  return { type: 'result', text, isError: true, error } as ProviderEvent;
}
...
export interface CursorResolvedConfiguration {
  ...
  mcpServers?: unknown;
  /**
   * Core passes this when the contract declares `configuration.tone`.
   * Cursor has no native tone control, so the provider declares none and
   * ignores the field. It stays on the mirror so this factory still accepts
      ...
      try {
        ...
        while (!aborted && pendingPrompts.length > 0) {
          ...
          if (result.status === 'cancelled') {
            ...
            yield failedResult('Cancelled');
            return;
          } else if (result.status === 'error') {
            // `text` is model output. The channel notice comes from `error`,
            // which the poll-loop delivers; model text is not that notice.
            const notice = result.error?.message || result.result || streamed || 'Cursor run failed';
            const modelText = result.error?.message ? result.result || streamed || null : null;
            yield failedResult(notice, modelText);
          } else {
        ...
        if (err instanceof CursorAgentError) {
          ...
          yield failedResult(err.message);
          return;
        }
        const message = err instanceof Error ? err.message : String(err);
        yield {
