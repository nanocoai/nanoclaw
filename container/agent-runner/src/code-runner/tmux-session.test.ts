...
describe('TmuxSession (fake exec)', () => {
  ...
  test('resume fallback: a first life dead before healthyRunMs respawns with the fallback pane command', async () => {
    const fake = fakeExec();
  ...
  test('resume fallback: a healthy first life keeps its argv — respawn-pane stays bare', async () => {
    const fake = fakeExec();
