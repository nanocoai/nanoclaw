...
const DEFAULT_SYNC_TIMING: SyncTiming = { minMs: 500, maxMs: 5_000 };
  ...
  private async materializeInboundAttachments(tables: InboundTables): Promise<void> {
    for (const message of tables.messages.records.values()) {
      ...
      for (const att of parsed.attachments) {
        if (typeof att.localPath !== 'string') continue;
        const parts = att.localPath.split('/');
        if (parts.length !== 3 || parts[0] !== 'inbox' || parts[1] !== message.id || !safePathSegment(parts[2])) continue;
        const target = path.join(workspaceRoot(), att.localPath);
        if (fs.existsSync(target)) continue;
        const response = await this.client.fetch(this.objectUrl(this.attachmentKey(message.id, parts[2])));
        if (response.status === 404) continue;
        await requireOk(response, this.options.bucket, this.attachmentKey(message.id, parts[2]));
        fs.mkdirSync(path.dirname(target), { recursive: true });
        fs.writeFileSync(target, Buffer.from(await response.arrayBuffer()), { flag: 'wx' });
      }
    }
  }
...
function workspaceRoot(): string {
  return process.env.NANOCLAW_WORKSPACE_ROOT?.trim() || '/workspace';
}
