 ...
 * the TLS dials that got past the guard. A dial that already carries a socket
 * is the HTTP/1.1 proxy tunnel's handshake, not a new TCP connection, and is
 * not recorded. Bun's fetch client dials natively and never appears here; an
 * HTTP/2 session does.
 */
...
(tls as { connect: typeof tls.connect }).connect = function (this: unknown, ...args: unknown[]) {
  const options = args.find((arg) => arg !== null && typeof arg === 'object') as
    | { host?: string; port?: string | number; ALPNProtocols?: unknown; socket?: unknown }
    | undefined;
  if (options?.socket) return (originalTlsConnect as unknown as (...a: unknown[]) => unknown).apply(this, args);
...
try {
  for await (const event of query.events) {
    if (event.type === 'error')
      ...
      const result = event as { text: string | null; isError?: boolean; error?: string };
      events.push({ type: 'result', text: result.text, isError: result.isError, error: result.error });
    }
