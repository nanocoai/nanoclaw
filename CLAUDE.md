...
| `/add-typesafe-tool` | Give agents TypeSafe's Jev decision model as the `typesafe-judge` container tool, with the `api.typesafe.ai` key in the gateway vault; basis of the `maintainer` template |
...
| [docs/typesafe-judge.md](docs/typesafe-judge.md) | `/add-typesafe-tool` (TypeSafe Jev judgments as a container tool, key in the gateway) and the `maintainer` agent template built on it |
