...
- **The minimum supported Node.js is now 22.14.0** (`engines` was `>=22`). better-sqlite3 13 prebuilds segfault on `new Database()` — a silent SIGSEGV with nothing on stderr — on Node 22 releases older than 22.14.0 ([WiseLibs/better-sqlite3#1514](https://github.com/WiseLibs/better-sqlite3/issues/1514)), so an affected Node passed every version check and then failed as hung test workers and a host that never starts. `setup.sh` and `setup/install-node.sh` now compare the minor version too, and the setup native-module check opens a throwaway database instead of only requiring the module, so the failure is reported at setup time with the actual cause. **Migration:** `node --version` — anything at or above v22.14.0 (including v24+) needs nothing; otherwise run `bash setup/install-node.sh` to upgrade.
