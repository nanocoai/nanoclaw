...
describe('session manager', () => {
  ...
  it('should sanitize a traversal-shaped messageId into a contained subdir rather than escaping the inbox', async () => {
    initSessionFolder('ag-1', 'sess-test');
    const { session } = await resolveSession('ag-1', 'mg-1', null, 'shared');
  ...
  it('should stage attachments for a Google Chat-style resource-path messageId', async () => {
    initSessionFolder('ag-1', 'sess-test');
    const { session } = await resolveSession('ag-1', 'mg-1', null, 'shared');
