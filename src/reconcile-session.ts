...
async function maintainSessionMailbox(
  ...
  // MODULE-HOOK:session-idle-reap:start
  {
    const { reapIdleSession } = await import('./session-idle-reap.js');
    reapIdleSession(mailbox, session, agentGroupId);
  }
  // MODULE-HOOK:session-idle-reap:end
