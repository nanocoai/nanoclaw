...
- **Fix: managed-fleet agents could no longer authenticate to the LLM (`401 No credentials config`).** A container reaching a link-local LLM gateway (configured by env vars, with OneCLI not in front of it) relied on the OneCLI proxy rewriting the `placeholder` bearer to the real gateway token — which the `@onecli-sh/sdk` 2.x credential model stopped doing for a plain host-pattern secret. The host `claude` provider now threads the operator's `NANOCLAW_ANTHROPIC_AUTH_TOKEN` (a non-secret gateway sentinel) into the container verbatim when set, so the agent presents it to the gateway directly. Standard OneCLI installs leave it unset and keep the placeholder for the proxy to overwrite, so the vault path is unchanged.
