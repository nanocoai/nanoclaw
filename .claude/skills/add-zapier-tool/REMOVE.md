...
## 1. Remove agent guidance

Use the skill-owned manager to remove every owned per-group source and
provider-materialized copy, including a copy left by a partial installation.
It leaves a same-named directory without the ownership marker untouched:

```bash
project_root=$(git rev-parse --show-toplevel)
skill_dir="${CLAUDE_SKILL_DIR:-$project_root/.claude/skills/add-zapier-tool}"
bash "$skill_dir/scripts/manage-zapier-skill.sh" remove "$project_root"
rm -f src/zapier-skill-manager.test.ts
```

## 2. Find affected groups
...
## 4. Remove skill-owned gateway rules
...
## 5. Reverse selective-agent grants
...
## 6. Remove the vault credential when unused
...
Restart only the groups recorded in step 2:
