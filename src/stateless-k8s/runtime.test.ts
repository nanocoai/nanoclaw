...
describe('stateless Kubernetes materialization', () => {
  ...
  it('requires the Pod driver and gives the agent no Host-backed mount', () => {
    ...
    // A file subPath over the group volume reads empty inside a Kata guest and
    // swallows the composer's write: neither file may be mounted, and the
    // config is projected read-only as a DIRECTORY instead.
    expect(mounts.find((mount) => mount.containerPath === '/workspace/agent/container.json')).toBeUndefined();
    expect(mounts.find((mount) => mount.containerPath === projectDocument.containerPath)).toBeUndefined();
    expect(mounts.find((mount) => mount.containerPath === '/run/nanoclaw/group-config'))
      .toMatchObject({ readonly: true, subPath: undefined });
...
describe('the composer DB URL read', () => {
  it('falls back from process.env to cwd/.env — the pod env does not carry NANOCLAW_DB_URL', () => {
    // Structural: the wiring goes red if the fallback is deleted. A behavior
    // test would need to chdir the whole suite (readEnvFile reads cwd/.env);
    // the env-set path is exercised by every composer test above.
    const source = fs.readFileSync(new URL('./runtime.ts', import.meta.url), 'utf8');
    expect(source).toContain("readEnvFile(['NANOCLAW_DB_URL'])");
    expect(source).toContain("import { readEnvFile } from '../env.js';");
  });
});
