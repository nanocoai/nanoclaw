...
while (($#)); do
  case "$1" in
    ...
    --consent-port) consent_port="${2:-}"; shift 2 ;;
    --portal-route) portal_route=1; shift ;;
    --admin-door) admin_door=1; shift ;;
    --sso-secret-id) sso_secret_id="${2:-}"; shift 2 ;;
    --sso-secret-region) sso_secret_region="${2:-}"; shift 2 ;;
    ...
    --governed-children) governed_children=1; shift ;;
...
# A deployment that composes no backlot facade passes no --mocks-namespace:
# there is no `mocks` namespace, and every backlot object below is skipped with
# it. This loop validated the empty default as though the flag were required,
# so the FIRST real installation (nancy-v3 -- real Slack, real Okta, no
# facades) could not run this installer at all: it printed usage and exited 2,
# which the driver grades as degraded, and the deployment came up with no
# Gateway and no HTTPRoute while every pod was healthy.
[[ "$system_namespace" =~ ^[a-z0-9]([-a-z0-9]*[a-z0-9])?$ ]] || usage
[[ -z "$mocks_namespace" || "$mocks_namespace" =~ ^[a-z0-9]([-a-z0-9]*[a-z0-9])?$ ]] || usage
for port in \
...
[[ -z "$consent_port" ]] || { [[ "$consent_port" =~ ^[0-9]+$ ]] && ((consent_port >= 1 && consent_port <= 65535)); } || usage
...
if [[ -n "$governance_development_actor" && -z "$admin_door" ]]; then
  # governance main (#317): proxy mode with a wall secret 401s any request
  # not carrying it, so the edge must stamp the secret beside the actor.
  # Read target-side with the instance role; Set replaces caller values.
  wall_secret_line=''
  if [[ -n "$sso_secret_id" ]]; then
    wall_secret="$(SSO_SECRET_ID="$sso_secret_id" AWS_DEFAULT_REGION="$sso_secret_region" python3 -c '
import json, os, boto3
d = json.loads(boto3.client("secretsmanager").get_secret_value(SecretId=os.environ["SSO_SECRET_ID"])["SecretString"])
print(d.get("GOVERNANCE_PROXY_HEADER_SECRET", ""))
')"
    [[ -n "$wall_secret" ]] && wall_secret_line="              - {name: X-Portal-Proxy-Secret, value: $wall_secret}"
  fi
...
# The consent callback route exists only where BOTH the edge and a consent
# port are stated — the same pairing the profile validator enforces for a real
# OAuth client. Path precedence gives /oauth to consent, everything else to
# the dashboard route above it.
consent_route_yaml=''
consent_traefik_netpol=''
portal_route_yaml=''
if [[ -n "$edge_domain" && -n "$portal_route" ]]; then
  IFS= read -r -d '' portal_route_yaml <<EOF || true
---
# The User Portal rides its own hostname to the oauth2-proxy Service. The
# consent route above it owns /oauth by PathPrefix precedence; /oauth2 (the
# sso callback) is a different first segment and stays here.
...
$(edge_parent_ref nanoco-system-edge)
  hostnames:
$(edge_hostname portal)
  rules:
    - backendRefs: [{name: portal, port: 80}]
EOF
fi

if [[ -n "$edge_domain" && -n "$consent_port" ]]; then
  consent_traefik_netpol=", {protocol: TCP, port: $consent_port}"
[[ -z "$admin_door" ]] || consent_traefik_netpol="$consent_traefik_netpol, {protocol: TCP, port: 4181}"
  IFS= read -r -d '' consent_route_yaml <<EOF || true
---
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
  name: governance-consent-edge
  namespace: $system_namespace
  labels: {app.kubernetes.io/managed-by: nanoco-tailnet-traefik}
spec:
  parentRefs:
...
$(edge_hostname portal)
  rules:
    - matches: [{path: {type: PathPrefix, value: /oauth}}]
      backendRefs: [{name: governance-consent, port: $consent_port}]
EOF
fi
...
# The backlot half of the manifest, composed only where a backlot exists. Held
# in variables rather than inlined so there is ONE writer for the mocks
# namespace: a deployment with no facade emits no Role, no RoleBinding, no
# Gateway, no product route and no NetworkPolicy for a namespace nothing
# creates.
mocks_rbac=''
mocks_routes=''
gateway_watch_namespaces="$system_namespace"
if [[ -n "$mocks_namespace" ]]; then
  gateway_watch_namespaces="$system_namespace,$mocks_namespace"
  mocks_rbac="$(cat <<EOF
---
...
$edge_entrypoint_arg
            ...
            - --providers.kubernetesgateway.namespaces=$gateway_watch_namespaces
...
$(edge_hostname governance)
  rules:
$(if [[ -n "$admin_door" ]]; then
  printf '    - backendRefs: [{name: governance-admin-sso, port: 4181}]\n'
else
  printf '    - backendRefs: [{name: governance-dashboard, port: %s}]\n' "$governance_port"
fi)
$governance_identity_filter
$consent_route_yaml
$portal_route_yaml
...
spec:
  ...
  ingress:
    - from:
        - namespaceSelector: {matchLabels: {kubernetes.io/metadata.name: kube-system}}
          podSelector: {matchLabels: {app.kubernetes.io/name: nanoco-tailnet-traefik}}
      ports: [{protocol: TCP, port: $governance_port}$consent_traefik_netpol]
    - from:
...
# The edge ROUTER and the governed-children wildcard route exist to reach
# governed child dev environments. They were gated on the edge domain alone, so
# a deployment that runs no dev environments still paid for a Deployment, a
# ConfigMap, a Service, a NetworkPolicy and a wildcard HTTPRoute that route to
# nothing. The profile already states whether dev environments exist; the driver
# passes --governed-children only when they do.
if [[ -n "$edge_domain" && -n "$governed_children" ]]; then
...
# Only a deployment that composes the facade has any of this to do. Without
# a backlot there is no Deployment to project origins into and no status
# endpoint to prove adoption against -- reaching for either is how a real
# installation died on a mock it never asked for.
if [[ -n "$mocks_namespace" ]]; then
...
if [[ -n "$edge_domain" && -n "$governed_children" ]]; then
  ...
  # Gating what is RENDERED does not remove what a previous revision applied:
  # `kubectl apply -f manifest` never prunes, so a deployment that stopped
  # composing dev environments kept the router Deployment, its Service,
  # ConfigMap, NetworkPolicy and the wildcard route until something deleted
  # them (nancy-v3, four hours after the gate landed). This is that something.
  "${kube[@]}" -n "$system_namespace" delete \
    deployment/nanoco-dev-env-edge-router service/nanoco-dev-env-edge-router \
    configmap/nanoco-dev-env-edge-router networkpolicy/nanoco-dev-env-edge-router \
    httproute/governed-children --ignore-not-found --wait=true >/dev/null
fi
...
[[ -z "$mocks_namespace" ]] || "${kube[@]}" -n "$mocks_namespace" wait --for=condition=Programmed gateway/nanoco-mocks-edge --timeout=120s >/dev/null
routes_ready=''
# How many routes THIS deployment composes, rather than how many the facade
# shape happens to have. The count used to be the literal 8-or-7 below, which
# is 6 backlot product routes plus governance-tailnet plus governed-children --
# so an installation with no facade composed 2, waited the full 120s for 8 that
# were never coming, and exited 1. Its routes were Accepted with resolved
# backends the whole time; only the arithmetic disagreed.
expected_routes=1
[[ -z "$edge_domain" || -z "$governed_children" ]] || expected_routes=$((expected_routes + 1))
[[ -z "$mocks_namespace" ]] || expected_routes=$((expected_routes + 6))
for _ in {1..60}; do
  routes="$("${kube[@]}" get httproute -A -l app.kubernetes.io/managed-by=nanoco-tailnet-traefik -o json)"
  # Every parent a route DECLARES must have attached, and every attachment must
  # be Accepted with resolved backends. Counting against a global expectation
  # instead was the same arithmetic mistake as the 8-or-7 above, one layer in:
  # an edge-only route (portal, the consent callback) declares one parentRef,
  # so demanding two graded a perfectly healthy deployment `degraded` forever.
  if jq -e --argjson expectedRoutes "$expected_routes" '
      [.items[] | select(.metadata.labels["nanoclaw.dev/env-id"] == null)] as $parentRoutes
      | ($parentRoutes | length) >= $expectedRoutes
      and all($parentRoutes[];
        (.status.parents | length) == (.spec.parentRefs | length)
...
# Only products this deployment composes. The backlot facades exist where
# --mocks-namespace was given; a real deployment has Governance alone, and a
# probe of `control` there answered HTTP 404 and graded the deploy degraded
# for a product it never had (nancy-v3, build 991).
loopback_probes=("governance|$governance_listen_port|/health")
if [[ -n "$mocks_namespace" ]]; then
  loopback_probes+=(
    ...
    "salesforce|$salesforce_listen_port|/"
  )
fi
for route_probe in "${loopback_probes[@]}"; do
...
if [[ -n "$edge_domain" ]]; then
  edge_probes=("governance|/health")
  if [[ -n "$mocks_namespace" ]]; then
    edge_probes+=("control|/healthz" "slack|/" "teams|/" "okta|/__mock/state" "google|/__mock/status" "salesforce|/")
  fi
  for route_probe in "${edge_probes[@]}"; do
...
serve_mappings=("443|$governance_listen_port")
if [[ -n "$mocks_namespace" ]]; then
  serve_mappings+=(
    ...
    "$backlot_salesforce_port|$salesforce_listen_port"
  )
fi
for serve_mapping in "${serve_mappings[@]}"; do
