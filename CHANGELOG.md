...
- **Messages wrapped in a tool-call turn are no longer silently dropped.** The Claude provider surfaced only the SDK's terminal `result` text, which carries just the final turn's content — so a `<message>` block emitted in an assistant turn that also made a tool call never reached the delivery layer, no error was raised, and the agent believed it had sent the message. `providers/claude.ts` now emits assistant text as it streams (a new `assistant_text` provider event) and `poll-loop.ts` falls back to it when a `result` arrives with no text of its own (ignoring it when the result has its own text, so nothing double-sends). Fixes handoffs and replies that vanished whenever the agent wrapped them in the same turn as a tool call.
