...
export function statelessAgentMounts(
  ...
  const mounts: VolumeMount[] = [
    ...
    // Kata-safe config projection. A FILE subPath over the group volume reads
    // as EMPTY inside the guest — and the composer writes through that same
    // shadowed view, so container.json and the project document both ended up
    // 0 bytes on the volume AND in the guest (nancy-v3, 2026-09-01: agents ran
    // with no config and no instructions, silently). The composer already
    // writes both into the group volume at their natural paths, so the file
    // mounts are deleted; the agent reads the project document in place and
    // NANOCLAW_CONTAINER_JSON resolves through this read-only DIRECTORY
    // projection at a non-overlapping path.
    volume(GROUP, '/run/nanoclaw/group-config', true, 'group-state', scope),
...
export function workspaceComposerContainer(input: WorkspaceComposerInput): ContainerSpec {
  ...
  const image = materializerImage(env);
  // env-then-.env, exactly as configuredWorkspaceReplicaRoot() reads its root:
  // the Host keeps its DB coordinates in cwd/.env (target custody), and the
  // pod env does not carry NANOCLAW_DB_URL — reading process.env alone made
  // every wake on the pod driver die here (measured on nancy-v3, 2026-08-30).
  const dbUrl = (env.NANOCLAW_DB_URL?.trim() || readEnvFile(['NANOCLAW_DB_URL']).NANOCLAW_DB_URL?.trim()) ?? '';
