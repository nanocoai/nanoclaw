...
// Base-owned test for the PreCompact capture seam. Pristine core ships NO registrant, so this
// proves the INERT contract (no-op + undefined timeout = upstream archive-only behaviour) plus
// the registered path + best-effort isolation. The TaskFlow memory-capture registrant is
...
describe('precompact-capture seam — inert on pristine core', () => {
